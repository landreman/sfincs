function [x,w,a,b,c] = sfincs_GaussWeightsAndAbscissae(N, weight, xMin, xMax, pointAtX0)
...
% together with the Jacobi matrix procedure of section 4.6.2.  See in
% particular the Stiel construction described in www.nr.com/webnotes?3
%
% Inputs:
% N = number of nodes
...
% X0 is the special location where we insist on having a grid point.
X0 = 0;
...
intervalDivider=10;
oldc=1;
lastPolynomialAtX0 = 0;
penultimatePolynomialAtX0 = 0;
for i=1:N
    j=i-1;
    if isinf(xMax)
...
diagonal = a;
if pointAtX0
    diagonal(end) = X0 - b(end)*penultimatePolynomialAtX0 / lastPolynomialAtX0;
end
JacobiMatrix = diag(diagonal) + diag(sqrtb,1) + diag(sqrtb,-1);
...
% The above algorithm often leaves |x(1)| ~ machine epsilon, so force x(1)=0:
if pointAtX0
    x(1) = 0;
end
