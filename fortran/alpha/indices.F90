  ...
  contains
    ...
    integer function getIndex(i_species, i_x, i_xi, i_alpha, i_zeta, whichBlock)

      ! This function takes as inputs "local" indices in the species, x, xi, alpha, and zeta grids,
      ...
      use globalVariables, only: Nspecies, Nx, Nxi, Nalpha, Nzeta, matrixSize, constraintScheme, includePhi1, masterProc, Nxi_for_x

      implicit none

      integer, intent(in) :: i_species, i_x, i_xi, i_alpha, i_zeta, whichBlock
      ...
      if (i_alpha < 1) then
         print *,"Error: i_alpha < 1"
         stop
      end if

      if (i_alpha > Nalpha) then
         print *,"Error: i_alpha > Nalpha"
         stop
      end if
      ...
      case (BLOCK_F)
!!$         getIndex = (i_species-1)*Nx*Nxi*Nalpha*Nzeta &
!!$              +(i_x-1)*Nxi*Nalpha*Nzeta &
!!$              +(i_xi-1)*Nalpha*Nzeta &
!!$              +(i_alpha-1)*Nzeta &
!!$              +i_zeta -1
         getIndex = (i_species-1)*DKE_size &
              +first_index_for_x(i_x)*Nalpha*Nzeta &
              +(i_xi-1)*Nalpha*Nzeta &
              +(i_alpha-1)*Nzeta &
              +i_zeta -1

      case (BLOCK_QN)
         !getIndex = Nspecies*Nx*Nxi*Nalpha*Nzeta &
    ...
    subroutine computeMatrixSize()
      ...
      use globalVariables, only: Nspecies, Nalpha, Nzeta, Nx, Nxi, includePhi1, constraintScheme, masterProc, matrixSize, Nxi_for_x

      implicit none

      integer :: ix

      DKE_size = sum(Nxi_for_x)*Nalpha*Nzeta

      matrixSize = Nspecies * DKE_size
      !matrixSize = Nspecies * Nalpha * Nzeta * Nxi * Nx
      ...
      if (includePhi1) then
         ! Include quasineutrality at Nalpha*Nzeta grid points, plus the <Phi_1>=0 constraint:
