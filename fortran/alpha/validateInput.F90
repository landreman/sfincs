...
!!        nonlinear = .false. !!Commented by AM 2016-02
  ...
  if (saveMatlabOutput .and. Nspecies*Nalpha*Nzeta*Nxi*Nx > 5000 .and. masterProc) then
     print *,line
     print *,line
...
!!$  end if 
  ...
  if (Nalpha*Nzeta*Nx*Nxi*Nspecies > 1e7 .and. masterProc) then
     print *,line
     print *,line
     print *,"**   WARNING: You have selected large resolution parameters, leading to a matrix size"
     print *,"**            exceeding 10 million x 10 million.  SFINCS may run out of memory."
     print *,line
     print *,line
  end if
  ...
  if (Nalpha<5) then
     if (masterProc) then
        print *,"Error! Nalpha must be at least 5."
     end if
     stop
  end if

  if (((Nalpha > 100 .and. Nzeta > 1) .or. (Nalpha>250)) .and. masterProc) then
     print *,line
     print *,line
     print *,"**   WARNING: You chose a very large value for Nalpha."
     print *,line
     print *,line
  end if
  ...
  if ((.not.forceOddNalphaAndNzeta ) .and. masterProc) then
     print *,line
     print *,line
     print *,"**   WARNING: forceOddNalphaAndNzeta = .true. is strongly recommended."
     print *,line
     print *,line
  end if
  ...
  if (alphaDerivativeScheme<0) then
     if (masterProc) then
        print *,"Error! alphaDerivativeScheme cannot be less than 0."
     end if
     stop
  end if
  
  if (alphaDerivativeScheme>2) then
     if (masterProc) then
        print *,"Error! alphaDerivativeScheme cannot be more than 2."
     end if
     stop
  end if
  
  if (alphaDerivativeScheme == 0 .and. masterProc) then
     print *,line
     print *,line
     print *,"**   WARNING: alphaDerivativeScheme=0 leads to very dense matrices,"
     print *,"**            meaning a lot of time and memory is required to solve the system."
     print *,"**            alphaDerivativeScheme = 2 is strongly recommended."
     print *,line
     print *,line
  end if

  if (alphaDerivativeScheme == 1 .and. masterProc) then
     print *,line
     print *,line
     print *,"**   WARNING: alphaDerivativeScheme=2 is typically preferred to alphaDerivativeScheme=1"
  ...
  if (zetaDerivativeScheme<1) then
     if (masterProc) then
        print *,"Error! zetaDerivativeScheme cannot be less than 1."
     end if
     stop
  end if
  ...
  if (ExBDerivativeSchemeAlpha<0) then
     if (masterProc) then
        print *,"Error! ExBDerivativeSchemeAlpha cannot be less than 0."
     end if
     stop
  end if
  
  if (ExBDerivativeSchemeAlpha>3) then
     if (masterProc) then
        print *,"Error! ExBDerivativeSchemeAlpha cannot be more than 3."
     end if
     stop
  end if
  ...
  if (ExBDerivativeSchemeAlpha>0 .and. preconditioner_alpha>0) then
     if (masterProc) then
        print *,"Error! The implementation of ExBDerivativeSchemeAlpha>0 does not presently allow"
        print *,"       preconditioning in alpha (preconditioner_alpha>0)."
     end if
     stop
  end if
...
!!$  if (magneticDriftDerivativeScheme>0 .and. preconditioner_alpha>0) then
!!$     if (masterProc) then
...
!!$  end if
  ...
  if (preconditioner_alpha<0) then
     if (masterProc) then
        print *,"Error! preconditioner_alpha cannot be less than 0."
     end if
     stop
  end if
  
  if (preconditioner_alpha>2) then
     if (masterProc) then
        print *,"Error! preconditioner_alpha cannot be more than 2."
     end if
     stop
  end if
  ...
  if (preconditioner_alpha_min_L<0) then
     if (masterProc) then
        print *,"Error! preconditioner_alpha_min_L should not be less than 0."
     end if
     stop
  end if
