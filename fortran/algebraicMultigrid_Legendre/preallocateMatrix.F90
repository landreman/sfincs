...
subroutine preallocateMatrix(matrix, whichMatrix)
  ...
  use globalVariables, only: Nx, Nxi, Ntheta, Nzeta, Nspecies, matrixSize, includePhi1, &
       ...
       ddtheta_plus, ddtheta_plus_preconditioner, ddtheta_sum, ddtheta_sum_preconditioner, ddtheta_difference, ddtheta_difference_preconditioner, &
       ddzeta_plus, ddzeta_plus_preconditioner, ddzeta_sum, ddzeta_sum_preconditioner, ddzeta_difference, ddzeta_difference_preconditioner
  use indices
...
!!$  ! We don't need to separately count the d/dxi terms, since they just add to the diagonals of the d/dtheta and d/dzeta terms we already counted.
  ...
  ! Diagonal-in-L terms, which include both streaming and ExB:
  tempInt1 = tempInt1 + max(max_nnz_per_row(Ntheta,ddtheta_plus + 100*ddtheta_difference), &
       max_nnz_per_row(Ntheta,ddtheta_plus_preconditioner + 100*ddtheta_difference_preconditioner)) - 1
  tempInt1 = tempInt1 + max(max_nnz_per_row(Nzeta,ddzeta_plus + 100*ddzeta_difference), &
       max_nnz_per_row(Nzeta,ddzeta_plus_preconditioner + 100*ddzeta_difference_preconditioner)) - 1

  ! Super-diagonal-in-L and sub-diagonal-in-L terms: (streaming + mirror)
  tempInt1 = tempInt1 + 2*max(max_nnz_per_row(Ntheta,ddtheta_sum), max_nnz_per_row(Ntheta,ddtheta_sum_preconditioner))
  tempInt1 = tempInt1 + 2*max(max_nnz_per_row(Nzeta,ddzeta_sum), max_nnz_per_row(Nzeta,ddzeta_sum_preconditioner))

  ! Super-super-diagonal-in-L and sub-sub-diagonal-in-L terms: (only streaming)
  tempInt1 = tempInt1 + 2*max(max_nnz_per_row(Ntheta,ddtheta_difference), max_nnz_per_row(Ntheta,ddtheta_difference_preconditioner))
  tempInt1 = tempInt1 + 2*max(max_nnz_per_row(Nzeta,ddzeta_difference), max_nnz_per_row(Nzeta,ddzeta_difference_preconditioner))
