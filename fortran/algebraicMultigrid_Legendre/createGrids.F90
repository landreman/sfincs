...
#include <petsc/finclude/petscdmdadef.h>
  ...
  subroutine createGrids()
    ...
    real(prec), dimension(:), allocatable :: temp_array
...
!    call PetscSynchronizedFlush(MPIComm, ierr)
    ...
    allocate(ddtheta_sum(Ntheta,Ntheta))
    allocate(ddtheta_difference(Ntheta,Ntheta))
    allocate(ddtheta_sum_preconditioner(Ntheta,Ntheta))
    allocate(ddtheta_difference_preconditioner(Ntheta,Ntheta))
    ...
    case (14)
       if (masterProc) then
          print *,"d/dtheta derivatives discretized using 5th order upwinded finite differences:"
          print *,"   3 points on either side."
          print *,"   upwinding factor:",upwinding_factor
       end if

       call_uniform_diff_matrices = .false.
       quadrature_option = 0
       derivative_option_plus  = 170
       call uniformDiffMatrices(Ntheta, zero, two*pi, derivative_option_plus,  quadrature_option, theta, thetaWeights, temp_matrix,  d2dtheta2)
       ddtheta_plus  = temp_matrix - upwinding_factor * d2dtheta2
       ddtheta_minus = temp_matrix + upwinding_factor * d2dtheta2
    ...
    case (12)
       if (masterProc) then
          ...
          print *,"   1 point on either side."
          print *,"   upwinding factor:",preconditioner_upwinding_factor
       end if
       ...
       derivative_option_plus  = 0
       call uniformDiffMatrices(Ntheta, zero, two*pi, derivative_option_plus,  quadrature_option, theta, thetaWeights, temp_matrix,  d2dtheta2)
       ddtheta_plus_preconditioner  = temp_matrix - preconditioner_upwinding_factor * d2dtheta2 * (theta(2) - theta(1))
       ddtheta_minus_preconditioner = temp_matrix + preconditioner_upwinding_factor * d2dtheta2 * (theta(2) - theta(1))
    ...
    case (13)
       ...
       derivative_option_plus  = 160
       call uniformDiffMatrices(Ntheta, zero, two*pi, derivative_option_plus,  quadrature_option, theta, thetaWeights, temp_matrix,  d2dtheta2)
       ddtheta_plus_preconditioner  = temp_matrix + preconditioner_upwinding_factor * d2dtheta2
       ddtheta_minus_preconditioner = temp_matrix - preconditioner_upwinding_factor * d2dtheta2

    case (14)
       if (masterProc) then
          print *,"Preconditioner d/dtheta derivatives discretized using 5th order upwinded finite differences:"
          print *,"   3 points on either side."
          print *,"   upwinding factor:",preconditioner_upwinding_factor
       end if
...
!!$       end if
    ddtheta_plus_preconditioner  = (1-preconditioner_blend)*ddtheta_plus_preconditioner  + preconditioner_blend*ddtheta_plus
...
!!$    end if
    ...
    allocate(ddzeta_sum(Nzeta,Nzeta))
    allocate(ddzeta_difference(Nzeta,Nzeta))
    allocate(ddzeta_sum_preconditioner(Nzeta,Nzeta))
    allocate(ddzeta_difference_preconditioner(Nzeta,Nzeta))
    ...
    if (Nzeta==1) then
       ...
       case (14)
          if (masterProc) then
             print *,"d/dzeta derivatives discretized using 5th order upwinded finite differences:"
             print *,"   3 points on either side."
             print *,"   upwinding factor:",upwinding_factor
          end if
       ...
       case (12)
          ...
          derivative_option_plus  = 0
          call uniformDiffMatrices(Nzeta, zero, zetaMax, derivative_option_plus,  quadrature_option, zeta, zetaWeights, temp_matrix,  d2dzeta2)
          ddzeta_plus_preconditioner  = temp_matrix - preconditioner_upwinding_factor * d2dzeta2 * (zeta(2) - zeta(1))
          ddzeta_minus_preconditioner = temp_matrix + preconditioner_upwinding_factor * d2dzeta2 * (zeta(2) - zeta(1))
       ...
       case (13)
          ...
          derivative_option_plus  = 160
          call uniformDiffMatrices(Nzeta, zero, zetaMax, derivative_option_plus,  quadrature_option, zeta, zetaWeights, temp_matrix,  d2dzeta2)
          ddzeta_plus_preconditioner  = temp_matrix + preconditioner_upwinding_factor * d2dzeta2
          ddzeta_minus_preconditioner = temp_matrix - preconditioner_upwinding_factor * d2dzeta2

       case (14)
          if (masterProc) then
             print *,"Preconditioner d/dzeta derivatives discretized using 5th order upwinded finite differences:"
             print *,"   3 points on either side."
             print *,"   upwinding factor:",preconditioner_upwinding_factor
          end if
...
!!$          end if
       ddzeta_plus_preconditioner  = (1-preconditioner_blend)*ddzeta_plus_preconditioner  + preconditioner_blend*ddzeta_plus
       ddzeta_minus_preconditioner = (1-preconditioner_blend)*ddzeta_minus_preconditioner + preconditioner_blend*ddzeta_minus
!!$       end if
...
!!$       end select
    ...
    ! Basis functions are defined as p_L(xi)= L_scaling(L+1) * P_L(xi)
    ! where P_L(xi) are the unscaled Legendre polynomials.
    ! The distribution function is expanded as f(xi) = \sum_L f_L p_L(xi)
    ! and the DKE is made dimensionless by applying the operation
    ! \int_{-1}^1 d\xi p_L (...).
    allocate(L_scaling(Nxi))
    select case (L_scaling_option)
    case (1)
       L_scaling = [( sqrt((2*L+one)/2), L=0, Nxi-1 )]
       ! The streaming term then scales as L^0, and the pitch angle scattering term scales as L^2.
    case (2)
       L_scaling = 1
       ! The streaming term then scales as L^{-1}, and the pitch angle scattering term scales as L^1.
    case (3)
       L_scaling = [( sqrt(2/(2*L+one)), L=0, Nxi-1 )]
       ! The streaming term then scales as L^{-2}, and the pitch angle scattering term scales as L^0.
    case default
       if (masterProc) print *,"Error! Invalid setting for L_scaling_option:",L_scaling_option
       stop
    end select
    ...
    call VecCreateMPI(PETSC_COMM_WORLD, PETSC_DECIDE, Ntheta*Nzeta*Nxi, null_vecs(1), ierr)
    call VecDuplicate(null_vecs(1), transpose_null_vecs(1), ierr)

    L = 0
    do itheta = ithetaMin, ithetaMax
       do izeta = izetaMin, izetaMax
          j = getIndex(1,1,L+1,itheta,izeta,BLOCK_F)
          call VecSetValue(null_vecs(1), j, one, INSERT_VALUES, ierr)
          call VecSetValue(transpose_null_vecs(1), j, sqrt_g(itheta,izeta)/spatial_scaling(itheta,izeta), INSERT_VALUES, ierr)
       end do
    end do
