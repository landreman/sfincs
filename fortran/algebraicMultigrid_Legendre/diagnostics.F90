             ...
             do izeta=1,Nzeta
                ...
                do ix=1,Nx
                   ...
                   densityPerturbation(ispecies,itheta,izeta) = densityPerturbation(ispecies,itheta,izeta) &
                        + densityFactor*xWeights(ix)*densityIntegralWeights(ix)*2*L_scaling(L+1)*solutionWithDeltaFArray(index)
                      
                   pressurePerturbation(ispecies,itheta,izeta) = pressurePerturbation(ispecies,itheta,izeta) &
                        + pressureFactor*xWeights(ix)*pressureIntegralWeights(ix)*2*L_scaling(L+1)*solutionWithDeltaFArray(index)
                      
                   particleFluxBeforeSurfaceIntegral_vm0(ispecies,itheta,izeta) &
                        = particleFluxBeforeSurfaceIntegral_vm0(ispecies,itheta,izeta) &
                        + (factor * (8/three) + factor2 * (two/3)) * particleFluxFactor_vm &
                        * xWeights(ix)*particleFluxIntegralWeights_vm(ix)*L_scaling(L+1)*f0Array(index)
                   
                   particleFluxBeforeSurfaceIntegral_vm(ispecies,itheta,izeta) &
                        = particleFluxBeforeSurfaceIntegral_vm(ispecies,itheta,izeta) &
                        + (factor * (8/three) + factor2 * (two/3)) * particleFluxFactor_vm &
                        * xWeights(ix)*particleFluxIntegralWeights_vm(ix)*L_scaling(L+1)*solutionWithFullFArray(index)
                   
                   particleFluxBeforeSurfaceIntegral_vE0(ispecies,itheta,izeta) &
                        = particleFluxBeforeSurfaceIntegral_vE0(ispecies,itheta,izeta) &
                        + factor_vE * particleFluxFactor_vE &
                        * xWeights(ix)*particleFluxIntegralWeights_vE(ix)*2*L_scaling(L+1)*f0Array(index)
                   
                   particleFluxBeforeSurfaceIntegral_vE(ispecies,itheta,izeta) &
                        = particleFluxBeforeSurfaceIntegral_vE(ispecies,itheta,izeta) &
                        + factor_vE * particleFluxFactor_vE &
                        * xWeights(ix)*particleFluxIntegralWeights_vE(ix)*2*L_scaling(L+1)*solutionWithFullFArray(index)
                   ...
                   heatFluxBeforeSurfaceIntegral_vm0(ispecies,itheta,izeta) &
                        = heatFluxBeforeSurfaceIntegral_vm0(ispecies,itheta,izeta) &
                        + (factor * (8/three) + factor2 * (two/3)) * heatFluxFactor_vm &
                        * xWeights(ix)*heatFluxIntegralWeights_vm(ix)*L_scaling(L+1)*f0Array(index)
                   
                   heatFluxBeforeSurfaceIntegral_vm(ispecies,itheta,izeta) &
                        = heatFluxBeforeSurfaceIntegral_vm(ispecies,itheta,izeta) &
                        + (factor * (8/three) + factor2 * (two/3)) * heatFluxFactor_vm &
                        * xWeights(ix)*heatFluxIntegralWeights_vm(ix)*L_scaling(L+1)*solutionWithFullFArray(index)
                   
                   heatFluxBeforeSurfaceIntegral_vE0(ispecies,itheta,izeta) &
                        = heatFluxBeforeSurfaceIntegral_vE0(ispecies,itheta,izeta) &
                        + factor_vE * heatFluxFactor_vE &
                        * xWeights(ix)*heatFluxIntegralWeights_vE(ix)*2*L_scaling(L+1)*f0Array(index)
                   
                   heatFluxBeforeSurfaceIntegral_vE(ispecies,itheta,izeta) &
                        = heatFluxBeforeSurfaceIntegral_vE(ispecies,itheta,izeta) &
                        + factor_vE * heatFluxFactor_vE &
                        * xWeights(ix)*heatFluxIntegralWeights_vE(ix)*2*L_scaling(L+1)*solutionWithFullFArray(index)
                   ...
                   flow(ispecies,itheta,izeta) = flow(ispecies,itheta,izeta) &
                        + flowFactor*xWeights(ix)*flowIntegralWeights(ix)*(two/3)*L_scaling(L+1)*solutionWithDeltaFArray(index)
