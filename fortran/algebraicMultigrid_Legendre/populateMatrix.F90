...
#include <petsc/finclude/petscmatdef.h>
  ...
  subroutine populateMatrix(matrix, whichMatrix, stateVec)
    ...
    real(prec), dimension(:,:), pointer :: ddtheta_to_use, ddzeta_to_use, ddx_to_use
    real(prec), dimension(:,:), pointer :: ddtheta_sum_to_use, ddzeta_sum_to_use, ddtheta_difference_to_use, ddzeta_difference_to_use
...
!!$       v_s = sqrt(THat/mHat) ! Using the version3 normalizations, there is no 2 here. But when I move to SI units, add a 2 inside the sqrt.
             ...
             do itheta = ithetaMin,ithetaMax
                do ix = 1,Nx
                   do L = 0, Nxi_for_x(ix)-1
                      factor = thermal_speeds(ispecies) * x(ix) * BHat_sup_zeta(itheta,izeta_row) / BHat(itheta,izeta_row) &
                           * spatial_scaling(itheta,izeta_row) * x_scaling(ix,ispecies) * L_scaling(L+1)
                      ...
                      do izeta_col = 1,Nzeta
                         ...
                         if (L < Nxi-1) then
                            ell = L + 1
                            L_factor = L_scaling(ell+1) * 2/(2*L+one) * (L+1)/(two*L+3)
                            colIndex = getIndex(ispecies, ix, ell+1, itheta, izeta_col, BLOCK_F)
                            call MatSetValueSparse(matrix, rowIndex, colIndex, &
                                 factor * L_factor * ddzeta_sum_to_use(izeta_row,izeta_col), ADD_VALUES, ierr)
                         end if

                         ! Sub-diagonal-in-L term
                         if (L > 0) then
                            ell = L - 1
                            L_factor = L_scaling(ell+1) * 2/(2*L+one) * L/(2*L-one)
                            colIndex = getIndex(ispecies, ix, ell+1, itheta, izeta_col, BLOCK_F)
                            call MatSetValueSparse(matrix, rowIndex, colIndex, &
                                 factor * L_factor * ddzeta_sum_to_use(izeta_row,izeta_col), ADD_VALUES, ierr)
                         end if
       ...
       ! *********************************************************
       ! Add the d/dtheta streaming term:
       ! *********************************************************
       ...
       if (whichMatrix .ne. 2) then
          do izeta = izetaMin,izetaMax
             do itheta_row = ithetaMin,ithetaMax
                do ix = 1,Nx
                   do L = 0, Nxi_for_x(ix)-1
                      factor = thermal_speeds(ispecies) * x(ix) * BHat_sup_theta(itheta_row,izeta) / BHat(itheta_row,izeta) &
                           * spatial_scaling(itheta_row,izeta) * x_scaling(ix,ispecies) * L_scaling(L+1)
                      ...
                      do itheta_col = 1,Ntheta
                         ...
                         if (L < Nxi-1) then
                            ...
                            colIndex = getIndex(ispecies, ix, ell+1, itheta_col, izeta, BLOCK_F)
                            call MatSetValueSparse(matrix, rowIndex, colIndex, &
                                 factor * L_factor * ddtheta_sum_to_use(itheta_row,itheta_col), ADD_VALUES, ierr)
                         end if
       ...
       if (whichMatrix .ne. 2) then
          do izeta = izetaMin,izetaMax
             do itheta = ithetaMin,ithetaMax
                do ix = 1,Nx
                   do L = 0, Nxi_for_x(ix)-1
                      factor = - thermal_speeds(ispecies)*x(ix)/(2*BHat(itheta,izeta)*BHat(itheta,izeta)) &
                           * (BHat_sup_theta(itheta,izeta)*dBHatdtheta(itheta,izeta) + BHat_sup_zeta(itheta,izeta)*dBHatdzeta(itheta,izeta)) &
                           * spatial_scaling(itheta,izeta) * x_scaling(ix,ispecies) * L_scaling(L+1)
                      ...
                      if (L < Nxi_for_x(ix)-1) then
                         ...
                         call MatSetValueSparse(matrix, rowIndex, colIndex, &
                              factor * L_factor, ADD_VALUES, ierr)
                      end if

                      ! Sub-diagonal term
                      if (L > 0) then
                         ell = L - 1
                         L_factor = L_scaling(ell+1) * 2/(2*L+one) * (-L)*(L-1)/(2*L-one)
                         colIndex = getIndex(ispecies, ix, ell+1, itheta, izeta, BLOCK_F)
                         call MatSetValueSparse(matrix, rowIndex, colIndex, &
                              factor * L_factor, ADD_VALUES, ierr)
                      end if
...
!!$       ! *********************************************************
!!$       ! Add the df/dxi term arising from Er:
!!$       ! *********************************************************
!!$
!!$       if (whichMatrix .ne. 2) then
!!$          do izeta = izetaMin,izetaMax
...
!!$          
!!$          
          stop "FP operator is not yet implemented"
...
!!$    end if
             ...
             do izeta = 1,Nzeta
                do ix = 1,Nx
                   do ispecies=1,Nspecies
                      ...
                      factor = thetaWeights(itheta)*zetaWeights(izeta)*sqrt_g(itheta,izeta)/VPrimeHat
    ...
    if (whichMatrix .ne. 2 .and. procThatHandlesConstraints .and. includePhi1) then
       rowIndex = getIndex(1, 1, 1, 1, 1, BLOCK_PHI1_CONSTRAINT)
       do itheta=1,Ntheta
          do izeta=1,Nzeta
             colIndex = getIndex(1, 1, 1, itheta, izeta, BLOCK_QN)
             call MatSetValueSparse(matrix, rowIndex, colIndex, &
                  thetaWeights(itheta)*zetaWeights(izeta)*sqrt_g(itheta,izeta), ADD_VALUES, ierr)
          end do
       end do
    end if
...
! -----------------------------------------------------------------------
  ...
  subroutine init_f0()
    ...
    integer :: ix, ixi, itheta, izeta, ispecies, index, L=0
    real(prec) :: factor
    PetscScalar :: scalar
    PetscErrorCode :: ierr
    ...
    ! Note f_0 = f_0 * P_0(xi) = f_0 * p_0(xi) / L_scaling(L+1)
    ! where p_L(xi) are the xi basis functions defined as p_L(xi)= L_scaling(L+1) * P_L(xi)
    ! where P_L(xi) are the unscaled Legendre polynomials.
    do ispecies = 1,Nspecies
       do ix = 1,Nx
          factor = expx2(ix) / L_scaling(L+1)
          do itheta = ithetaMin,ithetaMax
             do izeta = izetaMin,izetaMax
                index = getIndex(ispecies, ix, L+1, itheta, izeta, BLOCK_F)
