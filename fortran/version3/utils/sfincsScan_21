...
for paramname in runparamnames:
    ...
    if testReadName == None :
        if not(paramname in ["node","tasks_per_node","wall_clock_limit","nodes","ntasks-per-node","time"]):
            print("The variable " + paramname + 
                  " you wish to scan must be explicitly assigned some value in the appropriate namelist,")
...
for lind in range(numRunsInScan):
    oneline=""
...
for runNum in range(numRunsInScan):
   ...
   if sfincsSystem in ["draco", "eddy", "cobra", "raven", "viper", "marconi"]:
       for parind in range(len(runparamnames)):
           #SLURM parameters:
           if runparamnames[parind]=="nodes":
               for line in thisJobFile:
                   if "#SBATCH --nodes" in line:
                       thisJobFile[thisJobFile.index(line)]="#SBATCH --nodes="+str(runparams[runNum][parind])+"\n"
           if runparamnames[parind]=="ntasks-per-node":
               for line in thisJobFile:
                   if "#SBATCH --ntasks-per-node" in line:
                       thisJobFile[thisJobFile.index(line)]="#SBATCH --ntasks-per-node="+str(runparams[runNum][parind])+"\n"
           if runparamnames[parind]=="time":
               hours=runparams[runNum][parind]//60
               minutes=runparams[runNum][parind]%60
               hour_str=str(hours)
               minute_str=str(minutes)
               ...
               for line in thisJobFile:
                   if "#SBATCH --time" in line:
                       print("found!")
                       thisJobFile[thisJobFile.index(line)]="#SBATCH --time="+hour_str+":"+minute_str+":00\n"
