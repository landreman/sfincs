    ...
    case(2) ! BHat_sup_theta
      do itheta = 1,Ntheta
        do izeta = 1,Nzeta
          ...
          cos_angle = cos(angle)
          sin_angle = sin(angle)
          deltaBHat_sup_theta(itheta,izeta) = deltaLambda*bsupthetamnc_init(whichMode)*cos_angle
          deltadBHat_sup_theta_dzeta(itheta,izeta) = deltaLambda*bsupthetamnc_init(whichMode)*n*Nperiods*sin_angle
        end do
      end do
    ...
    case(3) ! BHat_sup_zeta
      do itheta = 1,Ntheta
        do izeta = 1,Nzeta
          ...
          cos_angle = cos(angle)
          sin_angle = sin(angle)
          deltaBHat_sup_zeta(itheta,izeta) = deltaLambda*bsupzetamnc_init(whichMode)*cos_angle
          deltadBHat_sup_zeta_dtheta(itheta,izeta) = -deltaLambda*bsupzetamnc_init(whichMode)*m*sin_angle
        end do
      end do
    ...
    case(4) ! BHat_sub_theta
      do itheta = 1,Ntheta
        do izeta = 1,Nzeta
          ...
          cos_angle = cos(angle)
          sin_angle = sin(angle)
          deltaBHat_sub_theta(itheta,izeta) = deltaLambda*bsubthetamnc_init(whichMode)*cos_angle
          deltadBHat_sub_theta_dzeta(itheta,izeta) = deltaLambda*bsubthetamnc_init(whichMode)*n*Nperiods*sin_angle
        end do
      end do
    ...
    case (5) ! BHat_sub_zeta
      do itheta = 1,Ntheta
        do izeta = 1,Nzeta
          ...
          cos_angle = cos(angle)
          sin_angle = sin(angle)
          deltaBHat_sub_zeta(itheta,izeta) = deltaLambda*bsubzetamnc_init(whichMode)*cos_angle
          deltadBHat_sub_zeta_dtheta(itheta,izeta) = -deltaLambda*bsubzetamnc_init(whichMode)*m*sin_angle
        end do
      end do
    ...
    case (6) ! DHat
      do itheta = 1,Ntheta
        do izeta = 1,Nzeta
          angle = m * theta(itheta) - n * NPeriods * zeta(izeta)
          cos_angle = cos(angle)
          deltaDHat(itheta,izeta) = one/(one/DHat(itheta,izeta) + deltaLambda*gmnc_init(whichMode)*cos_angle) - DHat(itheta,izeta)
        end do
      end do
      DHat = DHat + deltaDHat
    end select
