...
#include <petsc/finclude/petscmatdef.h>
  ...
  subroutine populateMatrix(matrix, whichMatrix, stateVec, level)
    ...
    real(prec), dimension(:,:), pointer :: BHat, dBHatdtheta, dBHatdzeta, dBHatdpsiHat, sqrt_g
    real(prec), dimension(:,:), pointer :: BHat_sub_psi, dBHat_sub_psi_dtheta, dBHat_sub_psi_dzeta
    real(prec), dimension(:,:), pointer :: BHat_sub_theta, dBHat_sub_theta_dzeta, dBHat_sub_theta_dpsiHat
    real(prec), dimension(:,:), pointer :: BHat_sub_zeta, dBHat_sub_zeta_dtheta, dBHat_sub_zeta_dpsiHat
    real(prec), dimension(:,:), pointer :: BHat_sup_theta, dBHat_sup_theta_dzeta, dBHat_sup_theta_dpsiHat
    real(prec), dimension(:,:), pointer :: BHat_sup_zeta, dBHat_sup_zeta_dtheta, dBHat_sup_zeta_dpsiHat
    real(prec), dimension(:,:), pointer :: BDotCurlB, uHat, gradpsidotgradB_overgpsipsi
    ...
    BHat => levels(level)%BHat
    dBHatdtheta => levels(level)%dBHatdtheta
    dBHatdzeta => levels(level)%dBHatdzeta
    dBHatdpsiHat => levels(level)%dBHatdpsiHat
    BHat_sub_psi => levels(level)%BHat_sub_psi
    dBHat_sub_psi_dtheta => levels(level)%dBHat_sub_psi_dtheta
    dBHat_sub_psi_dzeta => levels(level)%dBHat_sub_psi_dzeta
    BHat_sub_theta => levels(level)%BHat_sub_theta
    dBHat_sub_theta_dpsiHat => levels(level)%dBHat_sub_theta_dpsiHat
    dBHat_sub_theta_dzeta => levels(level)%dBHat_sub_theta_dzeta
    BHat_sub_zeta => levels(level)%BHat_sub_zeta
    dBHat_sub_zeta_dpsiHat => levels(level)%dBHat_sub_zeta_dpsiHat
    dBHat_sub_zeta_dtheta => levels(level)%dBHat_sub_zeta_dtheta
    BHat_sup_theta => levels(level)%BHat_sup_theta
    dBHat_sup_theta_dpsiHat => levels(level)%dBHat_sup_theta_dpsiHat
    dBHat_sup_theta_dzeta => levels(level)%dBHat_sup_theta_dzeta
    BHat_sup_zeta => levels(level)%BHat_sup_zeta
    dBHat_sup_zeta_dpsiHat => levels(level)%dBHat_sup_zeta_dpsiHat
    dBHat_sup_zeta_dtheta => levels(level)%dBHat_sup_zeta_dtheta
    sqrt_g => levels(level)%sqrt_g
    BDotCurlB => levels(level)%BDotCurlB
    uHat => levels(level)%uHat
    gradpsidotgradB_overgpsipsi => levels(level)%gradpsidotgradB_overgpsipsi
    ...
    do ispecies = 1,Nspecies
                   ...
                   do ixi = 1,levels(level)%Nxi_for_x(ix)
                      ...
                      if (whichMatrix == 1 .or. whichMatrix==3) then
                         ...
                         if (factor>0) then
                            ...
                            ddzeta_to_use => levels(level)%ddzeta_minus
                         end if
                      else
                         ! Preconditioner
                         if (factor>0) then
                            ddzeta_to_use => levels(level)%ddzeta_plus_preconditioner
                         else
                            ddzeta_to_use => levels(level)%ddzeta_minus_preconditioner
                         end if
                      end if
                      rowIndex = getIndex(level, ispecies, ix, ixi, itheta, izeta_row, BLOCK_F)
                      do izeta_col = 1,Nzeta
                         colIndex = getIndex(level, ispecies, ix, ixi, itheta, izeta_col, BLOCK_F)
                   ...
                   do ixi = 1,levels(level)%Nxi_for_x(ix)
                      ...
                      if (whichMatrix==1 .or. whichMatrix==3) then
                         ...
                         if (factor>0) then
                            ...
                            ddtheta_to_use => levels(level)%ddtheta_minus
                         end if
                      else
                         ! Preconditioner
                         if (factor>0) then
                            ddtheta_to_use => levels(level)%ddtheta_plus_preconditioner
                         else
                            ddtheta_to_use => levels(level)%ddtheta_minus_preconditioner
                         end if
                      end if

                      rowIndex = getIndex(level, ispecies, ix, ixi, itheta_row, izeta, BLOCK_F)
                      do itheta_col = 1,Ntheta
                         colIndex = getIndex(level, ispecies, ix, ixi, itheta_col, izeta, BLOCK_F)
                         call MatSetValueSparse(matrix, rowIndex, colIndex, &
       ...
       if (whichMatrix .ne. 2) then
          do izeta = izetaMin,izetaMax
             do itheta = ithetaMin,ithetaMax
                do ix = 1,Nx
                   do ixi_row = 1,levels(level)%Nxi_for_x(ix)
                      ...
                      if (whichMatrix==1 .or. whichMatrix==3) then
                         ...
                         if (factor>0) then
                            ...
                            ddxi_to_use => levels(level)%ddxi_minus
                         end if
                      else
                         ! Preconditioner
                         if (factor>0) then
                            ddxi_to_use => levels(level)%ddxi_plus_preconditioner
                         else
                            ddxi_to_use => levels(level)%ddxi_minus_preconditioner
                         end if
                      end if
                      rowIndex = getIndex(level, ispecies, ix, ixi_row, itheta, izeta, BLOCK_F)
                      do ixi_col = 1,levels(level)%Nxi_for_x(ix)
                         colIndex = getIndex(level, ispecies, ix, ixi_col, itheta, izeta, BLOCK_F)
                         call MatSetValueSparse(matrix, rowIndex, colIndex, &
             ...
             do itheta = ithetaMin,ithetaMax
                ...
                do ix_row = 1,Nx
                   do ixi = 1,Nxi
                      ...
                      rowIndex = getIndex(level, ispecies, ix_row, ixi, itheta, izeta, BLOCK_F)
                      do ix_col = 1,Nx
                         colIndex = getIndex(level, ispecies, ix_col, ixi, itheta, izeta, BLOCK_F)
                         call MatSetValueSparse(matrix, rowIndex, colIndex, &
...
!!$             do itheta_row = ithetaMin, ithetaMax
!!$                do izeta = izetaMin, izetaMax
!!$                   rowIndex = getIndex(level, ispecies, ix, L+1, itheta_row, izeta, BLOCK_F)
!!$
...
!!$
!!$                   do itheta_col = 1,Ntheta
!!$                      colIndex = getIndex(level,1,1,1,itheta_col, izeta, BLOCK_QN)
...
!!$             do itheta = ithetaMin, ithetaMax
!!$                do izeta_row = izetaMin, izetaMax
!!$                   rowIndex = getIndex(level, ispecies, ix, L+1, itheta, izeta_row, BLOCK_F)
!!$
!!$! COMMENTED BY AM 2016-02
...
!!$                   
!!$                   do izeta_col = 1,Nzeta
!!$                      colIndex = getIndex(level,1,1,1,itheta, izeta_col, BLOCK_QN)
!!$                      !!                      call MatSetValueSparse(matrix, rowIndex, colIndex,& !!Commented by AM 2016-03
...
!!$                do itheta = ithetaMin, ithetaMax
!!$                   do izeta = izetaMin, izetaMax
!!$                      rowIndex = getIndex(level, ispecies, ix, L+1, itheta, izeta, BLOCK_F)
!!$                      rowIndex2 = getIndex(level, ispecies, ix, L2+1, itheta, izeta, BLOCK_F) !!Added by AM 2016-03 to add extra P_2 terms
!!$                      
!!$                      !!These terms are diagonal in theta and zeta
!!$                      colIndex = getIndex(level,1,1,1,itheta, izeta, BLOCK_QN)
!!$                      
...
!!$                      rowIndex = getIndex(level,ispecies,ix_row,L+1,itheta,izeta,BLOCK_F)
!!$
...
!!$                               colIndex=getIndex(level,ispecies,ix_col,ell+1,itheta,izeta,BLOCK_F)
...
!!$                         index = getIndex(level,ispecies,ix,ell+1,itheta,izeta,BLOCK_F)
...
!!$                            colIndex = getIndex(level,1,1,1,j,izeta,BLOCK_QN)
!!$                            ! We must use MatSetValue instead of MatSetValueSparse here!!
!!$                            call MatSetValue(matrix, rowIndex, colIndex, &
...
!!$                            colIndex = getIndex(level,1,1,1,itheta,j,BLOCK_QN)
!!$                            ! We must use MatSetValue instead of MatSetValueSparse here!!
!!$                            call MatSetValue(matrix, rowIndex, colIndex, &
          ...
          if (NL>0) then
             ...
             elseif ((whichMatrix==0 .or. whichMatrix==4) .and. preconditioner_field_term_xi_option==2) then
                ...
                do ixi = 1,Nxi-1 ! Handle +/- 1 off-diagonal.
                   Legendre_projection_to_use(ixi,ixi+1,:) = levels(level)%Legendre_projection(ixi,ixi+1,:)
                   Legendre_projection_to_use(ixi+1,ixi,:) = levels(level)%Legendre_projection(ixi+1,ixi,:)
                end do
             else
                ! Keep full xi coupling.
                Legendre_projection_to_use = levels(level)%Legendre_projection
             end if
          end if
                      ...
                      do itheta = ithetaMin,ithetaMax
                         do izeta = izetaMin,izetaMax
                            ...
                            do ixi_col = 1,Nxi
                               ...
                               do ixi_row = 1,Nxi
                                  rowIndex = getIndex(level,iSpeciesA,ix_row,ixi_row,itheta,izeta,BLOCK_F)
                                  call MatSetValueSparse(matrix, rowIndex, colIndex, &
                ...
                do itheta=ithetaMin,ithetaMax
                   do izeta=izetaMin,izetaMax
                      ...
                      do ixi_row = 1,Nxi
                         ...
                         do ixi_col = 1,Nxi
                            colIndex=getIndex(level,iSpeciesA,ix,ixi_col,itheta,izeta,BLOCK_F)
                            call MatSetValueSparse(matrix, rowIndex, colIndex, &
...
!!$       ix = 1
!!$       do L = 1,(levels(level)%Nxi_for_x(ix)-1)
!!$          do itheta = ithetaMin,ithetaMax
!!$             do izeta = izetaMin,izetaMax
!!$                do ispecies = 1,Nspecies
!!$                   index = getIndex(level,ispecies,ix,L+1,itheta,izeta,BLOCK_F)
...
!!$                rowIndex = getIndex(level,ispecies,ix_row,L+1,itheta,izeta,BLOCK_F)
!!$                do ix_col = 1,Nx
!!$                   colIndex = getIndex(level,ispecies,ix_col,L+1,itheta,izeta,BLOCK_F)
...
!!$    end if
                ...
                do itheta = ithetaMin,ithetaMax
                   do izeta = izetaMin,izetaMax
                      ...
                      do ixi = 1,levels(level)%Nxi_for_x(ix)
                         ...
                         colIndex = getIndex(level, ispecies, 1, 1, 1, 1, BLOCK_DENSITY_CONSTRAINT)
                         call MatSetValueSparse(matrix, rowIndex, colIndex, factor*xPartOfSource1, ADD_VALUES, ierr)
                         
                         colIndex = getIndex(level, ispecies, 1, 1, 1, 1, BLOCK_PRESSURE_CONSTRAINT)
    ...
    !if (whichMatrix .ne. 2) then
       ...
       case (1,3,4)
          ...
          do ispecies=1,Nspecies
             rowIndices(1) = getIndex(level, ispecies, 1, 1, 1, 1, BLOCK_DENSITY_CONSTRAINT)
             rowIndices(2) = getIndex(level, ispecies, 1, 1, 1, 1, BLOCK_PRESSURE_CONSTRAINT)
             index = 1
             ...
             do ix=1,Nx
                do ixi=1,Nxi
                   ...
                   do itheta = 1,Ntheta
                      do izeta = 1,Nzeta
                         colIndices(index) = getIndex(level, ispecies, ix, ixi, itheta, izeta, BLOCK_F)
                         index = index + 1
                      end do
                   end do
                end do
       ...
       case (2)
          ...
          do ispecies = 1,Nspecies
             do ix=ixMin,Nx
                rowIndices = getIndex(level, ispecies, ix, 1, 1, 1, BLOCK_F_CONSTRAINT)
                index = 1
                ! It is critical that these next loops go in the same order as the loops for populating values_to_add above!
                do ixi=1,Nxi
                   !do itheta = ithetaMin,ithetaMax
    ...
    if (whichMatrix .ne. 2 .and. includePhi1) then
       ...
       do itheta = ithetaMin,ithetaMax
          do izeta = izetaMin,izetaMax
             rowIndex = getIndex(level, 1, 1, 1, itheta, izeta, BLOCK_QN)
             ...
             do ispecies = 1,Nspecies
                ...
                do ix = 1,Nx
                   colIndex = getIndex(level, ispecies, ix, L+1, itheta, izeta, BLOCK_F)
                   call MatSetValueSparse(matrix, rowIndex, colIndex, &
	     ...
	     !!!!!!!!!!!!!!!!!!!!!!!
             ...
             colIndex = getIndex(level, 1, 1, 1, 1, 1, BLOCK_PHI1_CONSTRAINT)
             call MatSetValueSparse(matrix, rowIndex, colIndex, &
                  one, ADD_VALUES, ierr)
    ...
    if ((whichMatrix==0 .or. whichMatrix==1 .or. whichMatrix==3) .and. procThatHandlesConstraints .and. includePhi1) then
       ...
       rowIndices(1) = getIndex(level, 1, 1, 1, 1, 1, BLOCK_PHI1_CONSTRAINT)
       do itheta=1,Ntheta
          do izeta=1,Nzeta
             colIndices(izeta) = getIndex(level, 1, 1, 1, itheta, izeta, BLOCK_QN)
          end do
    ...
    if (whichMatrix==4 .and. procThatHandlesConstraints) then
       ...
       case (1,3,4)
          ...
          do ispecies=1,Nspecies
             ...
             rowIndex = getIndex(level, ispecies, 1, 1, 1, 1, BLOCK_PRESSURE_CONSTRAINT)
             call MatSetValueSparse(matrix, rowIndex, rowIndex, one, ADD_VALUES, ierr)
          end do
       case (2)
          print *,"Adding 1s on the source/constraint diagonal block."
