...
#include <petsc/finclude/petscsysdef.h>
  ...
  integer(HSIZE_T), dimension(2) :: dimForThetaTheta
  ...
  integer(HID_T) :: dspaceIDForThetaTheta
...
contains
  ...
  subroutine initializeOutputFile()
    ...
    if (masterProc) then
       ...
       call writeHDF5Field("theta", levels(1)%theta, dspaceIDForTheta, dimForTheta, &
            "Grid points in the field line label coordinate, which runs from 0 to 2pi")
       call writeHDF5Field("zeta", levels(1)%zeta, dspaceIDForZeta, dimForZeta, &
       ...
       call writeHDF5Field("xi", levels(1)%xi, dspaceIDForXi, dimForXi, &
            "Grid points for cosine of the pitch angle, v||/v.")
       call writeHDF5Field("xiWeights", levels(1)%xiWeights, dspaceIDForXi, dimForXi, &
            "Quadrature weights in xi")
       call writeHDF5Field("ddtheta_plus", levels(1)%ddtheta_plus, dspaceIDforThetaTheta, dimForThetaTheta, "")
       call writeHDF5Field("ddtheta_minus", levels(1)%ddtheta_minus, dspaceIDforThetaTheta, dimForThetaTheta, "")
       call writeHDF5Field("ddtheta_plus_preconditioner", levels(1)%ddtheta_plus_preconditioner, dspaceIDforThetaTheta, dimForThetaTheta, "")
       call writeHDF5Field("ddtheta_minus_preconditioner", levels(1)%ddtheta_minus_preconditioner, dspaceIDforThetaTheta, dimForThetaTheta, "")
       call writeHDF5Field("ddzeta_plus", levels(1)%ddzeta_plus, dspaceIDforZetaZeta, dimForZetaZeta, "")
       call writeHDF5Field("ddzeta_minus", levels(1)%ddzeta_minus, dspaceIDforZetaZeta, dimForZetaZeta, "")
       call writeHDF5Field("ddzeta_plus_preconditioner", levels(1)%ddzeta_plus_preconditioner, dspaceIDforZetaZeta, dimForZetaZeta, "")
       call writeHDF5Field("ddzeta_minus_preconditioner", levels(1)%ddzeta_minus_preconditioner, dspaceIDforZetaZeta, dimForZetaZeta, "")
       call writeHDF5Field("ddxi_plus", levels(1)%ddxi_plus, dspaceIDforxixi, dimForxixi, "")
       call writeHDF5Field("ddxi_minus", levels(1)%ddxi_minus, dspaceIDforxixi, dimForxixi, "")
       call writeHDF5Field("ddxi_plus_preconditioner", levels(1)%ddxi_plus_preconditioner, dspaceIDforxixi, dimForxixi, "")
       ...
       call writeHDF5Field("preconditioner_theta_derivative_option", preconditioner_theta_derivative_option, "")
       ...
       call writeHDF5Field("sqrt_g", levels(1)%sqrt_g, dspaceIDForThetaZeta, dimForThetaZeta, &
            "Jacobian 1/(grad psi dot grad theta cross grad zeta), where 2*pi*psi is the toroidal flux)")
       call writeHDF5Field("BHat", levels(1)%BHat, dspaceIDForThetaZeta, dimForThetaZeta, &
            "Magnitude of the magnetic field, in units of BBar")
       call writeHDF5Field("dBHatdpsiHat", levels(1)%dBHatdpsiHat, dspaceIDForThetaZeta, dimForThetaZeta, "")
       call writeHDF5Field("dBHatdtheta", levels(1)%dBHatdtheta, dspaceIDForThetaZeta, dimForThetaZeta, "")
       call writeHDF5Field("dBHatdzeta", levels(1)%dBHatdzeta, dspaceIDForThetaZeta, dimForThetaZeta, "")
       call writeHDF5Field("BDotCurlB", levels(1)%BDotCurlB, dspaceIDForThetaZeta, dimForThetaZeta, &
            "\vect{B}\cdot\nabla\times\vect{B}, in units of BBar^2 / RBar")
       call writeHDF5Field("uHat", levels(1)%uHat, dspaceIDForThetaZeta, dimForThetaZeta, &
            "\nabla_\parallel u = (2/B^4)\nabla B\times\vector{B}\cdot\iota\nabla\psi")

       call writeHDF5Field("BHat_sub_psi", levels(1)%BHat_sub_psi, dspaceIDForThetaZeta, dimForThetaZeta, &
            "Covariant component of B, the component that multiplies grad psi, in units of 1 / RBar")
       call writeHDF5Field("dBHat_sub_psi_dtheta", levels(1)%dBHat_sub_psi_dtheta, dspaceIDForThetaZeta, dimForThetaZeta, "")
       call writeHDF5Field("dBHat_sub_psi_dzeta", levels(1)%dBHat_sub_psi_dzeta, dspaceIDForThetaZeta, dimForThetaZeta, "")

       call writeHDF5Field("BHat_sub_theta", levels(1)%BHat_sub_theta, dspaceIDForThetaZeta, dimForThetaZeta, &
            "Covariant component of B, the component that multiplies grad theta, in units of BBar * RBar")
       call writeHDF5Field("dBHat_sub_theta_dpsiHat", levels(1)%dBHat_sub_theta_dpsiHat, dspaceIDForThetaZeta, dimForThetaZeta, "")
       call writeHDF5Field("dBHat_sub_theta_dzeta", levels(1)%dBHat_sub_theta_dzeta, dspaceIDForThetaZeta, dimForThetaZeta, "")

       call writeHDF5Field("BHat_sub_zeta", levels(1)%BHat_sub_zeta, dspaceIDForThetaZeta, dimForThetaZeta, &
            "Covariant component of B, the component that multiplies grad zeta, in units of BBar * RBar")
       call writeHDF5Field("dBHat_sub_zeta_dpsiHat", levels(1)%dBHat_sub_zeta_dpsiHat, dspaceIDForThetaZeta, dimForThetaZeta, "")
       call writeHDF5Field("dBHat_sub_zeta_dtheta", levels(1)%dBHat_sub_zeta_dtheta, dspaceIDForThetaZeta, dimForThetaZeta, "")

       call writeHDF5Field("BHat_sup_theta", levels(1)%BHat_sup_theta, dspaceIDForThetaZeta, dimForThetaZeta, &
            "Contravariant component of B, the component that multiplies d r / d theta, in units of BBar / RBar")
       call writeHDF5Field("dBHat_sup_theta_dpsiHat", levels(1)%dBHat_sup_theta_dpsiHat, dspaceIDForThetaZeta, dimForThetaZeta, "")
       call writeHDF5Field("dBHat_sup_theta_dzeta", levels(1)%dBHat_sup_theta_dzeta, dspaceIDForThetaZeta, dimForThetaZeta, "")

       call writeHDF5Field("BHat_sup_zeta", levels(1)%BHat_sup_zeta, dspaceIDForThetaZeta, dimForThetaZeta, &
            "Contravariant component of B, the component that multiplies d r / d zeta, in units of BBar / RBar")
       call writeHDF5Field("dBHat_sup_zeta_dpsiHat", levels(1)%dBHat_sup_zeta_dpsiHat, dspaceIDForThetaZeta, dimForThetaZeta, "")
       call writeHDF5Field("dBHat_sup_zeta_dtheta", levels(1)%dBHat_sup_zeta_dtheta, dspaceIDForThetaZeta, dimForThetaZeta, "")
...
! export_f has not yet been updated to reflect the new theta coordinate
...
!!$       end if
       ...
       call h5sclose_f(dspaceIDForThetaTheta, HDF5Error)
...
!!$    call h5screate_simple_f(rank, dimForExport_f_x, dspaceIDForExport_f_x, HDF5Error)
    ...
    dimForThetaTheta(1) = Ntheta
    dimForThetaTheta(2) = Ntheta
    call h5screate_simple_f(rank, dimForThetaTheta, dspaceIDForThetaTheta, HDF5Error)
...
#define maxInputFileSize 99999
  ...
  subroutine writeHDF5Doubles2(arrayName, data, dspaceID, dims, description)
    ...
    elseif (dspaceID == dspaceIDForThetaTheta) then
       call h5dsset_label_f(dsetID, 1, "theta", HDF5Error)
       call h5dsset_label_f(dsetID, 2, "theta", HDF5Error)
