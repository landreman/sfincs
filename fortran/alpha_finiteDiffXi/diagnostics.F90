...
! This next subroutine is called as a "Monitor" of SNES, set in solver.F90 using SNESSetMonitor.
  ...
  subroutine extractPhi1(myVec)
    ...
    use globalVariables, only: Phi1Hat, dPhi1Hatdalpha, dPhi1Hatdzeta, MPIComm, masterProc, ddalpha_plus, ddzeta_plus, Nalpha, Nzeta
    ...
    if (includePhi1) then
       ...
       if (masterProc) then
          ...
          do ialpha = 1,Nalpha
             do izeta = 1,Nzeta
                index = getIndex(1,1,1,ialpha,izeta,BLOCK_QN)+1
                ! Add 1 because getIndex returns 0-based PETSc indices, not 1-based fortran indices.
                Phi1Hat(ialpha,izeta) = solnarray(index)
             end do
          end do
       ...
       dPhi1Hatdalpha = matmul(ddalpha_plus,Phi1Hat)
  ...
  subroutine diagnostics(solutionWithDeltaF, iterationNum)
    ...
    integer :: i, j, ix, ixi, ispecies, ialpha, izeta, L, index
    ...
    integer :: ialpha1, izeta1, ixi1, ix1
    integer :: ialpha2, izeta2, ixi2, ix2
    ...
    !!!                 exp(-Zs(ispecies)*gamma*Phi1Hat(ialpha,izeta)/THats(ispecies)), INSERT_VALUES, ierr)
    !!!         end do
    !!!      end do
    !!!   end do
    !!!end do
...
!!$          heatFluxFactor_vE = 2*gamma*pi*Delta*THat*THat*sqrtTHat/(2*VPrimeHat*mHat*sqrtMHat)
          ...
          do ialpha=1,Nalpha
             do izeta=1,Nzeta
                ...
                !factor = (GHat*dBHatdtheta(ialpha,izeta) - IHat*dBHatdzeta(ialpha,izeta))/(BHat(ialpha,izeta) ** 3)

                factor = (BHat_sub_theta(ialpha,izeta) * dBHatdzeta(ialpha,izeta) &
                     - BHat_sub_zeta(ialpha,izeta) * dBHatdtheta(ialpha,izeta)) / (BHat(ialpha,izeta) ** 3)
                ...
                if (force0RadialCurrentInEquilibrium) then
                   ...
                   factor2 = 2 * (dBHat_sub_zeta_dtheta(ialpha,izeta) - dBHat_sub_theta_dzeta(ialpha,izeta)) &
                        / (BHat(ialpha,izeta) ** 2)
                end if

                factor_vE = (BHat_sub_theta(ialpha,izeta) * dPhi1Hatdzeta(ialpha,izeta) &
                     -BHat_sub_zeta(ialpha,izeta) * dPhi1Hatdalpha(ialpha,izeta)) &
                     / (BHat(ialpha,izeta) ** 2)

                do ix=1,Nx
                   do ixi = 1,Nxi
                      index = getIndex(ispecies, ix, ixi, ialpha, izeta, BLOCK_F)+1
                      ! Add 1 to index to convert from PETSc 0-based index to fortran 1-based index.

                      densityPerturbation(ispecies,ialpha,izeta) = densityPerturbation(ispecies,ialpha,izeta) &
                           + densityFactor*xWeights(ix)*densityIntegralWeights(ix)*xiWeights(ixi)*solutionWithDeltaFArray(index)
                      ...
                      FSABFlow_vs_x(ispecies,ix) = FSABFlow_vs_x(ispecies,ix) &
                           + flowFactor*xi(ixi)*xWeights(ix)*flowIntegralWeights(ix)*xiWeights(ixi)*solutionWithDeltaFArray(index) &
                           * alphaWeights(ialpha) * zetaWeights(izeta) * BHat(ialpha,izeta) / DHat(ialpha,izeta)
                      ...
                      particleFluxBeforeSurfaceIntegral_vm0(ispecies,ialpha,izeta) &
                           = particleFluxBeforeSurfaceIntegral_vm0(ispecies,ialpha,izeta) &
                      ...
                      particleFluxBeforeSurfaceIntegral_vm(ispecies,ialpha,izeta) &
                           = particleFluxBeforeSurfaceIntegral_vm(ispecies,ialpha,izeta) &
                           + (factor * (1+xi(ixi)*xi(ixi)) + factor2 * (xi(ixi)*xi(ixi))) * particleFluxFactor_vm &
                           * xWeights(ix)*particleFluxIntegralWeights_vm(ix)*xiWeights(ixi)*solutionWithFullFArray(index)

                      particleFluxBeforeSurfaceIntegral_vE0(ispecies,ialpha,izeta) &
                           = particleFluxBeforeSurfaceIntegral_vE0(ispecies,ialpha,izeta) &
                           + factor_vE * particleFluxFactor_vE &
                           * xWeights(ix)*particleFluxIntegralWeights_vE(ix)*xiWeights(ixi)*f0Array(index)
                      
                      particleFluxBeforeSurfaceIntegral_vE(ispecies,ialpha,izeta) &
                           = particleFluxBeforeSurfaceIntegral_vE(ispecies,ialpha,izeta) &
                           + factor_vE * particleFluxFactor_vE &
                           * xWeights(ix)*particleFluxIntegralWeights_vE(ix)*xiWeights(ixi)*solutionWithFullFArray(index)
                      
                      !print *,particleFluxBeforeSurfaceIntegral_vE0(ispecies,ialpha,izeta),particleFluxBeforeSurfaceIntegral_vE(ispecies,ialpha,izeta)
                      
                      heatFluxBeforeSurfaceIntegral_vm0(ispecies,ialpha,izeta) &
                           = heatFluxBeforeSurfaceIntegral_vm0(ispecies,ialpha,izeta) &
                      ...
                      heatFluxBeforeSurfaceIntegral_vm(ispecies,ialpha,izeta) &
                           = heatFluxBeforeSurfaceIntegral_vm(ispecies,ialpha,izeta) &
                           + (factor * (1+xi(ixi)*xi(ixi)) + factor2 * (xi(ixi)*xi(ixi))) * heatFluxFactor_vm &
                           * xWeights(ix)*heatFluxIntegralWeights_vm(ix)*xiWeights(ixi)*solutionWithFullFArray(index)
                      
                      heatFluxBeforeSurfaceIntegral_vE0(ispecies,ialpha,izeta) &
                           = heatFluxBeforeSurfaceIntegral_vE0(ispecies,ialpha,izeta) &
                           + factor_vE * heatFluxFactor_vE &
                           * xWeights(ix)*heatFluxIntegralWeights_vE(ix)*xiWeights(ixi)*f0Array(index)
                      
                      heatFluxBeforeSurfaceIntegral_vE(ispecies,ialpha,izeta) &
                           = heatFluxBeforeSurfaceIntegral_vE(ispecies,ialpha,izeta) &
                           + factor_vE * heatFluxFactor_vE &
                           * xWeights(ix)*heatFluxIntegralWeights_vE(ix)*xiWeights(ixi)*solutionWithFullFArray(index)
                      
                      particleFlux_vm_psiHat_vs_x(ispecies,ix) &
                           = particleFlux_vm_psiHat_vs_x(ispecies,ix) &
                           + (factor * (1+xi(ixi)*xi(ixi)) + factor2 * (xi(ixi)*xi(ixi))) * particleFluxFactor_vm &
                           * xWeights(ix)*particleFluxIntegralWeights_vm(ix)*xiWeights(ixi)*solutionWithFullFArray(index) &
                           * alphaWeights(ialpha) * zetaWeights(izeta)
...
!!$
!!$                      L = 1
!!$                      index = getIndex(ispecies, ix, L+1, ialpha, izeta, BLOCK_F)+1
!!$                      ! Add 1 to index to convert from PETSc 0-based index to fortran 1-based index.
!!$                      
!!$                      
!!$                   momentumFluxBeforeSurfaceIntegral_vm0(ispecies,ialpha,izeta) &
!!$                        = momentumFluxBeforeSurfaceIntegral_vm0(ispecies,ialpha,izeta) &
!!$                        + (factor * (16d+0/15) + factor2 * (two/5)) * momentumFluxFactor_vm * BHat(ialpha,izeta) &
!!$                        * xWeights(ix)*momentumFluxIntegralWeights_vm(ix)*f0Array(index)
!!$
!!$                   momentumFluxBeforeSurfaceIntegral_vm(ispecies,ialpha,izeta) &
!!$                        = momentumFluxBeforeSurfaceIntegral_vm(ispecies,ialpha,izeta) &
!!$                        + (factor * (16d+0/15) + factor2 * (two/5)) * momentumFluxFactor_vm * BHat(ialpha,izeta) &
!!$                        * xWeights(ix)*momentumFluxIntegralWeights_vm(ix)*solutionWithFullFArray(index)
!!$
!!$                   momentumFluxBeforeSurfaceIntegral_vE0(ispecies,ialpha,izeta) &
!!$                        = momentumFluxBeforeSurfaceIntegral_vE0(ispecies,ialpha,izeta) &
!!$                        + factor_vE * (two/3) * momentumFluxFactor_vE * BHat(ialpha,izeta) &
!!$                        * xWeights(ix)*momentumFluxIntegralWeights_vE(ix)*f0Array(index)
!!$
!!$                   momentumFluxBeforeSurfaceIntegral_vE(ispecies,ialpha,izeta) &
!!$                        = momentumFluxBeforeSurfaceIntegral_vE(ispecies,ialpha,izeta) &
!!$                        + factor_vE * (two/3) * momentumFluxFactor_vE * BHat(ialpha,izeta) &
...
!!$
!!$                   NTVBeforeSurfaceIntegral(ispecies,ialpha,izeta) &
!!$                        = NTVBeforeSurfaceIntegral(ispecies,ialpha,izeta) &
!!$                        + NTVFactor * NTVKernel(ialpha,izeta)&
!!$                        * xWeights(ix)*NTVIntegralWeights(ix)*xiWeights(ixi)*solutionWithDeltaFArray(index)
!!$
!!$                   pressureAnisotropy(ispecies,ialpha,izeta) = pressureAnisotropy(ispecies,ialpha,izeta) &
!!$                        + pressureFactor*(-three/5)* &
!!$			xWeights(ix)*pressureIntegralWeights(ix)*xiWeights(ixi)*solutionWithDeltaFArray(index)
!!$
!!$                   particleFlux_vm_psiHat_vs_x(ispecies,ix) &
!!$                        = particleFlux_vm_psiHat_vs_x(ispecies,ix) &
!!$                        + (factor+factor2) * (four/15) * particleFluxFactor_vm &
...
!!$                   momentumFluxBeforeSurfaceIntegral_vm0(ispecies,ialpha,izeta) &
!!$                        = momentumFluxBeforeSurfaceIntegral_vm0(ispecies,ialpha,izeta) &
!!$                        + (factor+factor2) * (four/35) * momentumFluxFactor_vm * BHat(ialpha,izeta) &
!!$                        * xWeights(ix)*momentumFluxIntegralWeights_vm(ix)*f0Array(index)
!!$
!!$                   momentumFluxBeforeSurfaceIntegral_vm(ispecies,ialpha,izeta) &
!!$                        = momentumFluxBeforeSurfaceIntegral_vm(ispecies,ialpha,izeta) &
...
!!$                        * xWeights(ix)*momentumFluxIntegralWeights_vm(ix)*solutionWithFullFArray(index)
          ...
          do izeta=1,Nzeta
             ...
             FSADensityPerturbation(ispecies) = FSADensityPerturbation(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, densityPerturbation(ispecies,:,izeta)/DHat(:,izeta))

             FSABFlow(ispecies) = FSABFlow(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, flow(ispecies,:,izeta)*BHat(:,izeta)/DHat(:,izeta))

             FSAPressurePerturbation(ispecies) = FSAPressurePerturbation(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, pressurePerturbation(ispecies,:,izeta)/DHat(:,izeta))

             particleFlux_vm0_psiHat(ispecies) = particleFlux_vm0_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, particleFluxBeforeSurfaceIntegral_vm0(ispecies,:,izeta))

             particleFlux_vm_psiHat(ispecies) = particleFlux_vm_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, particleFluxBeforeSurfaceIntegral_vm(ispecies,:,izeta))

             particleFlux_vE0_psiHat(ispecies) = particleFlux_vE0_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, particleFluxBeforeSurfaceIntegral_vE0(ispecies,:,izeta))

             particleFlux_vE_psiHat(ispecies) = particleFlux_vE_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, particleFluxBeforeSurfaceIntegral_vE(ispecies,:,izeta))

             momentumFlux_vm0_psiHat(ispecies) = momentumFlux_vm0_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, momentumFluxBeforeSurfaceIntegral_vm0(ispecies,:,izeta))

             momentumFlux_vm_psiHat(ispecies) = momentumFlux_vm_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, momentumFluxBeforeSurfaceIntegral_vm(ispecies,:,izeta))

             momentumFlux_vE0_psiHat(ispecies) = momentumFlux_vE0_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, momentumFluxBeforeSurfaceIntegral_vE0(ispecies,:,izeta))

             momentumFlux_vE_psiHat(ispecies) = momentumFlux_vE_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, momentumFluxBeforeSurfaceIntegral_vE(ispecies,:,izeta))

             heatFlux_vm0_psiHat(ispecies) = heatFlux_vm0_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, heatFluxBeforeSurfaceIntegral_vm0(ispecies,:,izeta))

             heatFlux_vm_psiHat(ispecies) = heatFlux_vm_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, heatFluxBeforeSurfaceIntegral_vm(ispecies,:,izeta))

             heatFlux_vE0_psiHat(ispecies) = heatFlux_vE0_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, heatFluxBeforeSurfaceIntegral_vE0(ispecies,:,izeta))

             heatFlux_vE_psiHat(ispecies) = heatFlux_vE_psiHat(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, heatFluxBeforeSurfaceIntegral_vE(ispecies,:,izeta))

             NTV(ispecies) = NTV(ispecies) + zetaWeights(izeta) &
                  * dot_product(alphaWeights, NTVBeforeSurfaceIntegral(ispecies,:,izeta)) 
...
!!$                         index = getIndex(ispecies, ix1, ixi1, ialpha1, izeta1, BLOCK_F)+1
!!$                         temp1 = solutionWithFullFArray(index)
!!$                         do ialpha2 = 1,N_export_f_theta
!!$                            temp2 = temp1 * map_theta_to_export_f_theta(ialpha2, ialpha1)
...
!!$                                  full_f(ispecies, ialpha2, izeta2, :, ix2) = &
!!$                                       full_f(ispecies, ialpha2, izeta2, :, ix2) + temp3 &
...
!!$                                  ! I arbitrarily chose to replace the loop over export_f_xi with ":"
!!$                                  ! We could pick any of the 4 coordinates for this.
!!$                                  delta_f(ispecies, ialpha2, izeta2, :, ix2) = &
!!$                                       delta_f(ispecies, ialpha2, izeta2, :, ix2) + temp3 &
!!$                                       * map_x_to_export_f_x(ix2, ix1) &
!!$                                       * map_xi_to_export_f_xi(:, ixi1)
!!$                               end do
!!$                            end do
