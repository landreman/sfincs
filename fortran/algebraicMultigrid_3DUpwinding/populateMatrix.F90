...
#include <petsc/finclude/petscmatdef.h>
  ...
  subroutine populateMatrix(matrix, whichMatrix, stateVec)
    ...
    ! ************************************************************
    ! Compute the deflection frequency for the pitch angle scattering operator
    ! ************************************************************
    nu_D = zero
    ...
    if (whichMatrix>0) then
       ! Not preconditioner
       first_derivative_stencil_to_use = first_derivative_stencil
       second_derivative_stencil_to_use = second_derivative_stencil
    else
       ! Preconditioner
       first_derivative_stencil_to_use = first_derivative_stencil_preconditioner
       second_derivative_stencil_to_use = second_derivative_stencil_preconditioner
    end if
       ...
       do ix = 1, Nx
          do itheta_row = ithetaMin, ithetaMax
             do izeta_row = izetaMin, izetaMax
                do ixi_row = 1, Nxi
                   rowIndex = getIndex(ispecies, ix, ixi_row, itheta_row, izeta_row, BLOCK_F)

                   ! Streaming term:
                   theta_dot = thermal_speeds(ispecies) * x(ix) * xi(ixi_row) * BHat_sup_theta(itheta_row,izeta_row) / BHat(itheta_row,izeta_row)
                   zeta_dot  = thermal_speeds(ispecies) * x(ix) * xi(ixi_row) * BHat_sup_zeta( itheta_row,izeta_row) / BHat(itheta_row,izeta_row)

                   ! Add the ExB term:
                   select case (ExB_option)
                   case (1)
                      theta_dot = theta_dot + BHat_sub_zeta( itheta_row,izeta_row)/(sqrt_g(itheta_row,izeta_row)*BHat(itheta_row,izeta_row)*BHat(itheta_row,izeta_row))*dPhiHatdpsiHat
                      zeta_dot  = zeta_dot  - BHat_sub_theta(itheta_row,izeta_row)/(sqrt_g(itheta_row,izeta_row)*BHat(itheta_row,izeta_row)*BHat(itheta_row,izeta_row))*dPhiHatdpsiHat
                   case (2)
                      theta_dot = theta_dot + BHat_sub_zeta( itheta_row,izeta_row)/(sqrt_g(itheta_row,izeta_row)*FSABHat2)*dPhiHatdpsiHat
                      zeta_dot  = zeta_dot  - BHat_sub_theta(itheta_row,izeta_row)/(sqrt_g(itheta_row,izeta_row)*FSABHat2)*dPhiHatdpsiHat
                   end select

                   ! Mirror term
                   alpha_dot = - thermal_speeds(ispecies)*x(ix)*(1-xi(ixi_row)*xi(ixi_row))/(2*BHat(itheta_row,izeta_row)*BHat(itheta_row,izeta_row)) &
                        * (BHat_sup_theta(itheta_row,izeta_row)*dBHatdtheta(itheta_row,izeta_row) + BHat_sup_zeta(itheta_row,izeta_row)*dBHatdzeta(itheta_row,izeta_row)) &
                        / sin_alpha(ixi_row)
                   ! 1st-derivative part of the pitch-angle-scattering operator
                   alpha_dot = alpha_dot + (nu_D(ispecies,ix)/2) * xi(ixi_row) / sin_alpha(ixi_row)
                   ! Er term in alpha dot:
                   if (includeElectricFieldTermInXiDot) then
                      alpha_dot = alpha_dot + xi(ixi_row)*(1-xi(ixi_row)*xi(ixi_row)) &
                           / (2*sqrt_g(itheta_row,izeta_row)*BHat(itheta_row,izeta_row)*BHat(itheta_row,izeta_row)*BHat(itheta_row,izeta_row)) &
                           * (BHat_sub_zeta(itheta,izeta)*dBHatdtheta(itheta,izeta) - BHat_sub_theta(itheta_row,izeta_row)*dBHatdzeta(itheta_row,izeta_row))*dPhiHatdpsiHat &
                           / sin_alpha(ixi_row)
                   end if

!!$                   ! Scale
!!$                   theta_dot = theta_dot * spatial_scaling(itheta_row,izeta_row) * x_scaling(ix,ispecies) * xi_scaling(ixi_row) 
!!$                   zeta_dot  = zeta_dot  * spatial_scaling(itheta_row,izeta_row) * x_scaling(ix,ispecies) * xi_scaling(ixi_row) 
!!$                   alpha_dot = alpha_dot * spatial_scaling(itheta_row,izeta_row) * x_scaling(ix,ispecies) * xi_scaling(ixi_row) 
!!$
!!$                   abs_v_theta_over_dtheta = abs(theta_dot / dtheta)
...
!!$                   abs_v_alpha_over_dalpha = abs(alpha_dot / dalpha)
                   ...
                   case (0)
                      ...
                      zeta_increment(1) = 0
                      alpha_increment(1) = 0
                      derivative_magnitude(1) = abs_v_theta_over_dtheta
                   ...
                   case (1)
                      if (abs_v_theta_over_dtheta > abs_v_zeta_over_dzeta .and. abs_v_theta_over_dtheta > abs_v_alpha_over_dalpha) then
                         ...
                         if (abs_v_theta_over_dtheta > abs_v_zeta_over_dzeta + abs_v_alpha_over_dalpha) then
                            ...
                            derivative_magnitude(1) = abs_v_theta_over_dtheta - abs_v_zeta_over_dzeta - abs_v_alpha_over_dalpha
                            derivative_magnitude(2) = abs_v_zeta_over_dzeta
                            derivative_magnitude(3) = abs_v_alpha_over_dalpha
                         else
                            ...
                            derivative_magnitude(1) = abs_v_zeta_over_dzeta + abs_v_alpha_over_dalpha - abs_v_theta_over_dtheta
                            derivative_magnitude(2) = abs_v_theta_over_dtheta - abs_v_alpha_over_dalpha
                            derivative_magnitude(3) = abs_v_theta_over_dtheta - abs_v_zeta_over_dzeta
                         end if
                         theta_increment(2) = 1
                         zeta_increment(2)  = 1
                         alpha_increment(2) = 0
                      ...
                      elseif (abs_v_zeta_over_dzeta > abs_v_theta_over_dtheta .and. abs_v_zeta_over_dzeta > abs_v_alpha_over_dalpha) then
                         ...
                         if (abs_v_zeta_over_dzeta > abs_v_theta_over_dtheta + abs_v_alpha_over_dalpha) then
                            ...
                            derivative_magnitude(1) = abs_v_zeta_over_dzeta - abs_v_theta_over_dtheta - abs_v_alpha_over_dalpha
                            ...
                            derivative_magnitude(1) = abs_v_theta_over_dtheta + abs_v_alpha_over_dalpha - abs_v_zeta_over_dzeta
                            derivative_magnitude(2) = abs_v_zeta_over_dzeta - abs_v_alpha_over_dalpha
                            derivative_magnitude(3) = abs_v_zeta_over_dzeta - abs_v_theta_over_dtheta
                         end if
                         theta_increment(2) = 1
                         zeta_increment(2)  = 1
                         alpha_increment(2) = 0
                         ...
                         if (abs_v_alpha_over_dalpha > abs_v_theta_over_dtheta + abs_v_zeta_over_dzeta) then
                            ...
                            derivative_magnitude(1) = abs_v_alpha_over_dalpha - abs_v_theta_over_dtheta - abs_v_zeta_over_dzeta
                            derivative_magnitude(2) = abs_v_theta_over_dtheta
                            ...
                            derivative_magnitude(1) = abs_v_theta_over_dtheta + abs_v_zeta_over_dzeta - abs_v_alpha_over_dalpha
                            derivative_magnitude(2) = abs_v_alpha_over_dalpha - abs_v_zeta_over_dzeta
                            derivative_magnitude(3) = abs_v_alpha_over_dalpha - abs_v_theta_over_dtheta
                         end if
                         theta_increment(2) = 1
                         zeta_increment(2)  = 0
                         alpha_increment(2) = 1
                   ...
                   do j=1,3
                      do stencil_index = -stencil_width, stencil_width
                         ...
                         ixi_col    =    ixi_row + alpha_increment(j)*stencil_index
                         ! Handle cases in which the stencil crosses the domain boundaries
                         itheta_col = modulo(itheta_col - 1, Ntheta) + 1 ! Periodic. Note that we must use modulo rather than mod!
                         izeta_col  = modulo(izeta_col  - 1, Nzeta ) + 1 ! Periodic. Note that we must use modulo rather than mod!
!!$                         if (itheta_col < 1) itheta_col = itheta_col + Ntheta ! Periodic
!!$                         if (itheta_col > Ntheta) itheta_col = itheta_col - Ntheta ! Periodic
!!$                         if (izeta_col < 1) izeta_col = izeta_col + Nzeta ! Periodic
!!$                         if (izeta_col > Nzeta) izeta_col = izeta_col - Nzeta ! Periodic
                         if (ixi_col < 1) ixi_col = 1 - ixi_col ! f(alpha) = f(-alpha)
                         if (ixi_col > Nxi) ixi_col = 2*Nxi + 1 - ixi_col ! f(alpha) = f(2*pi-alpha)

                         ! Add the matrix element:
                         colIndex = getIndex(ispecies, ix, ixi_col, itheta_col, izeta_col, BLOCK_F)
                         call MatSetValueSparse(matrix, rowIndex, colIndex, &
                              derivative_magnitude(j) * first_derivative_stencil_to_use(stencil_index), ADD_VALUES, ierr)
                      end do
                   end do

                   ! Add d^2/dalpha^2 part of pitch angle scattering
                   do stencil_index = -stencil_width, stencil_width
                      itheta_col = itheta_row
                      izeta_col = izeta_row
                      ixi_col = ixi_row + stencil_index
                      if (ixi_col < 1) ixi_col = 1 - ixi_col ! f(alpha) = f(-alpha)
                      if (ixi_col > Nxi) ixi_col = 2*Nxi + 1 - ixi_col ! f(alpha) = f(2*pi-alpha)

                      ! Add the matrix element:
                      factor = - nu_D(ispecies,ix) / (2 * dalpha * dalpha) * spatial_scaling(itheta_row,izeta_row) * x_scaling(ix,ispecies) * xi_scaling(ixi_row)
                      colIndex = getIndex(ispecies, ix, ixi_col, itheta_col, izeta_col, BLOCK_F)
                      call MatSetValueSparse(matrix, rowIndex, colIndex, &
                           factor * second_derivative_stencil_to_use(stencil_index), ADD_VALUES, ierr)
                   end do
                end do ! xi
             end do ! zeta
          end do ! theta
...
!!$       end if
    ...
    call PetscTime(time4, ierr)
    if (masterProc) print *,"  Time for main DKE terms:",time4-time3
    call PetscTime(time3, ierr)
             ...
             do ixi=1,Nxi
                ...
                do itheta = 1,Ntheta
                   do izeta = 1,Nzeta
                      ...
                      factor = xi_weights(ixi)*theta_weight*zeta_weight*sqrt_g(itheta,izeta)/VPrimeHat
    ...
    if (whichMatrix .ne. 2 .and. procThatHandlesConstraints .and. includePhi1) then
       ...
       do itheta=1,Ntheta
          ...
          call MatSetValuesSparse(matrix, 1, rowIndices(1), Nzeta, colIndices, &
               theta_weight*zeta_weight*sqrt_g(itheta,:), ADD_VALUES, ierr)
       end do
