...
! This next subroutine is called as a "Monitor" of SNES, set in solver.F90 using SNESSetMonitor.
  ...
  subroutine extractPhi1(myVec)
    ...
    use globalVariables, only: Phi1Hat, dPhi1Hatdtheta, dPhi1Hatdzeta, MPIComm, masterProc, Ntheta, Nzeta
                ...
                do ix=1,Nx
                   do ixi = 1,Nxi
                      ...
                      densityPerturbation(ispecies,itheta,izeta) = densityPerturbation(ispecies,itheta,izeta) &
                           + densityFactor*xWeights(ix)*densityIntegralWeights(ix)*xi_weights(ixi)*solutionWithDeltaFArray(index)
                      
                      flow(ispecies,itheta,izeta) = flow(ispecies,itheta,izeta) &
                           + flowFactor*xi(ixi)*xWeights(ix)*flowIntegralWeights(ix)*xi_weights(ixi)*solutionWithDeltaFArray(index)
                      
                      FSABFlow_vs_x(ispecies,ix) = FSABFlow_vs_x(ispecies,ix) &
                           + flowFactor*xi(ixi)*xWeights(ix)*flowIntegralWeights(ix)*xi_weights(ixi)*solutionWithDeltaFArray(index) &
                           * theta_weight * zeta_weight * BHat(itheta,izeta) * sqrt_g(itheta,izeta)

                      pressurePerturbation(ispecies,itheta,izeta) = pressurePerturbation(ispecies,itheta,izeta) &
                           + pressureFactor*xWeights(ix)*pressureIntegralWeights(ix)*xi_weights(ixi)*solutionWithDeltaFArray(index)
                      ...
                      particleFluxBeforeSurfaceIntegral_vm0(ispecies,itheta,izeta) &
                           ...
                           * xWeights(ix)*particleFluxIntegralWeights_vm(ix)*xi_weights(ixi)*f0Array(index)
                      
                      particleFluxBeforeSurfaceIntegral_vm(ispecies,itheta,izeta) &
                           = particleFluxBeforeSurfaceIntegral_vm(ispecies,itheta,izeta) &
                           + (factor * (1+xi(ixi)*xi(ixi)) + factor2 * (xi(ixi)*xi(ixi))) * particleFluxFactor_vm &
                           * xWeights(ix)*particleFluxIntegralWeights_vm(ix)*xi_weights(ixi)*solutionWithFullFArray(index)

                      particleFluxBeforeSurfaceIntegral_vE0(ispecies,itheta,izeta) &
                           = particleFluxBeforeSurfaceIntegral_vE0(ispecies,itheta,izeta) &
                           + factor_vE * particleFluxFactor_vE &
                           * xWeights(ix)*particleFluxIntegralWeights_vE(ix)*xi_weights(ixi)*f0Array(index)
                      
                      particleFluxBeforeSurfaceIntegral_vE(ispecies,itheta,izeta) &
                           = particleFluxBeforeSurfaceIntegral_vE(ispecies,itheta,izeta) &
                           + factor_vE * particleFluxFactor_vE &
                           * xWeights(ix)*particleFluxIntegralWeights_vE(ix)*xi_weights(ixi)*solutionWithFullFArray(index)
                      ...
                      heatFluxBeforeSurfaceIntegral_vm0(ispecies,itheta,izeta) &
                           ...
                           * xWeights(ix)*heatFluxIntegralWeights_vm(ix)*xi_weights(ixi)*f0Array(index)
                      
                      heatFluxBeforeSurfaceIntegral_vm(ispecies,itheta,izeta) &
                           = heatFluxBeforeSurfaceIntegral_vm(ispecies,itheta,izeta) &
                           + (factor * (1+xi(ixi)*xi(ixi)) + factor2 * (xi(ixi)*xi(ixi))) * heatFluxFactor_vm &
                           * xWeights(ix)*heatFluxIntegralWeights_vm(ix)*xi_weights(ixi)*solutionWithFullFArray(index)
                      
                      heatFluxBeforeSurfaceIntegral_vE0(ispecies,itheta,izeta) &
                           = heatFluxBeforeSurfaceIntegral_vE0(ispecies,itheta,izeta) &
                           + factor_vE * heatFluxFactor_vE &
                           * xWeights(ix)*heatFluxIntegralWeights_vE(ix)*xi_weights(ixi)*f0Array(index)
                      
                      heatFluxBeforeSurfaceIntegral_vE(ispecies,itheta,izeta) &
                           = heatFluxBeforeSurfaceIntegral_vE(ispecies,itheta,izeta) &
                           + factor_vE * heatFluxFactor_vE &
                           * xWeights(ix)*heatFluxIntegralWeights_vE(ix)*xi_weights(ixi)*solutionWithFullFArray(index)
                      
                      particleFlux_vm_psiHat_vs_x(ispecies,ix) &
                           = particleFlux_vm_psiHat_vs_x(ispecies,ix) &
                           + (factor * (1+xi(ixi)*xi(ixi)) + factor2 * (xi(ixi)*xi(ixi))) * particleFluxFactor_vm &
                           * xWeights(ix)*particleFluxIntegralWeights_vm(ix)*xi_weights(ixi)*solutionWithFullFArray(index) &
                           * theta_weight * zeta_weight
...
!!$                        * xWeights(ix)*NTVIntegralWeights(ix)*xi_weights(ixi)*solutionWithDeltaFArray(index)
!!$
!!$                   pressureAnisotropy(ispecies,itheta,izeta) = pressureAnisotropy(ispecies,itheta,izeta) &
!!$                        + pressureFactor*(-three/5)* &
!!$			xWeights(ix)*pressureIntegralWeights(ix)*xi_weights(ixi)*solutionWithDeltaFArray(index)
!!$
!!$                   particleFlux_vm_psiHat_vs_x(ispecies,ix) &
!!$                        = particleFlux_vm_psiHat_vs_x(ispecies,ix) &
!!$                        + (factor+factor2) * (four/15) * particleFluxFactor_vm &
...
!!$                        * xWeights(ix)*momentumFluxIntegralWeights_vm(ix)*solutionWithFullFArray(index)
          ...
          do izeta=1,Nzeta
             ...
             FSADensityPerturbation(ispecies) = FSADensityPerturbation(ispecies) + zeta_weight &
                  * theta_weight * sum(densityPerturbation(ispecies,:,izeta) * sqrt_g(:,izeta))

             FSABFlow(ispecies) = FSABFlow(ispecies) + zeta_weight &
                  * theta_weight * sum(flow(ispecies,:,izeta)*BHat(:,izeta) * sqrt_g(:,izeta))

             FSAPressurePerturbation(ispecies) = FSAPressurePerturbation(ispecies) + zeta_weight &
                  * theta_weight * sum(pressurePerturbation(ispecies,:,izeta) * sqrt_g(:,izeta))

             particleFlux_vm0_psiHat(ispecies) = particleFlux_vm0_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(particleFluxBeforeSurfaceIntegral_vm0(ispecies,:,izeta))

             particleFlux_vm_psiHat(ispecies) = particleFlux_vm_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(particleFluxBeforeSurfaceIntegral_vm(ispecies,:,izeta))

             particleFlux_vE0_psiHat(ispecies) = particleFlux_vE0_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(particleFluxBeforeSurfaceIntegral_vE0(ispecies,:,izeta))

             particleFlux_vE_psiHat(ispecies) = particleFlux_vE_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(particleFluxBeforeSurfaceIntegral_vE(ispecies,:,izeta))

             momentumFlux_vm0_psiHat(ispecies) = momentumFlux_vm0_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(momentumFluxBeforeSurfaceIntegral_vm0(ispecies,:,izeta))

             momentumFlux_vm_psiHat(ispecies) = momentumFlux_vm_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(momentumFluxBeforeSurfaceIntegral_vm(ispecies,:,izeta))

             momentumFlux_vE0_psiHat(ispecies) = momentumFlux_vE0_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(momentumFluxBeforeSurfaceIntegral_vE0(ispecies,:,izeta))

             momentumFlux_vE_psiHat(ispecies) = momentumFlux_vE_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(momentumFluxBeforeSurfaceIntegral_vE(ispecies,:,izeta))

             heatFlux_vm0_psiHat(ispecies) = heatFlux_vm0_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(heatFluxBeforeSurfaceIntegral_vm0(ispecies,:,izeta))

             heatFlux_vm_psiHat(ispecies) = heatFlux_vm_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(heatFluxBeforeSurfaceIntegral_vm(ispecies,:,izeta))

             heatFlux_vE0_psiHat(ispecies) = heatFlux_vE0_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(heatFluxBeforeSurfaceIntegral_vE0(ispecies,:,izeta))

             heatFlux_vE_psiHat(ispecies) = heatFlux_vE_psiHat(ispecies) + zeta_weight &
                  * theta_weight * sum(heatFluxBeforeSurfaceIntegral_vE(ispecies,:,izeta))

             NTV(ispecies) = NTV(ispecies) + zeta_weight &
                  * theta_weight * sum(NTVBeforeSurfaceIntegral(ispecies,:,izeta)) 
