...
{Boolean}
{\false}
{Whenever \parlink{RHSMode} == 1 and \parlink{includePhi1} == \true and \parlink{quasineutralityOption} == 1}
{When this is \true, an extra species (here called NBIspec)
  is added to the quasineutrality condition, but not as a species for
  which the DKE is solved. This is to mimic that fast ions influece
  the other species mostly via the parallel electric field, an not
  much via collisions. The NBIspec is assumed to have a density
  distribution on the flux suface proporional to $B$ (which is the
  case if all particles have zero pitch angle).}
...
{1.0}
{Whenever \parlink{RHSMode} == 1 and \parlink{includePhi1} == \true and \parlink{quasineutralityOption} == 1}
{Charge of the fast ion species, in units of the proton charge $e$.}
...
{0.0}
{Whenever \parlink{RHSMode} == 1 and \parlink{includePhi1} == \true and \parlink{quasineutralityOption} == 1}
{Density of the fast ion species, in units of the reference density $\bar{n}$.}
...
{1D array of reals}
{0.0}
{\todo{Whenever \parlink{RHSMode} == 1} ?}
{\todo{Write description}}
