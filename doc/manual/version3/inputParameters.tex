...
{1}
{When \parlink{ambipolarSolve} == \true.}
{Indicates which root solving algorithm to use for ambipolar solve. If \parlink{ambipolarSolveOption}==1 a Newton bisection method is used, \parlink{ambipolarSolveOption}==2 a Brent method is used, and if \parlink{ambipolarSolveOption}==3 a Newton method is used. If \parlink{ambipolarSolveOption}==1 or 3, an adjoint method will used to compute the $\partial J_r/\partial E_r$, so must be used with \parlink{includePhi1}==\false, \parlink{EParallelHat}==0, \parlink{magneticDriftScheme}>0, \parlink{constraintScheme}==-1 or 1, and \parlink{collisionOperator}==0.}
...
{\ttfamily geometryScheme}==13: Read Boozer geometry from the {\ttfamily geometryParamters} namelist. Here the geometry parameters are specified by \parlink{boozer\_bmnc}, \parlink{GHat}, \parlink{IHat}, and \parlink{iota}. This has been implemented for use with STELLOPT for direct optimization of the Boozer spectrum. 
}
...
{real}
{3.7481}
{Only when \parlink{geometryScheme} == 1 or 13. Otherwise, {\ttfamily GHat} will be set according to the requested \parlink{geometryScheme}.}
...
{real}
{0.0}
{Only when \parlink{geometryScheme} == 1 or 13. Otherwise, {\ttfamily IHat} will be set according to the requested \parlink{geometryScheme}.}
...
{real}
{0.4542}
{Only when \parlink{geometryScheme} == 1 or 13. Otherwise, {\ttfamily iota} will be set according to the requested \parlink{geometryScheme}.}
...
{real}
{1.0}
{Only when \parlink{geometryScheme}==5. \parlink{helicity\_l} and \parlink{helicity\_n} must be specified.}
{Scales the values of components of VMEC geometry (modes of $B$, $B^{u}$, $B_{u}$, $B^{v}$, $B_{v}$, $B_{s}$, and $\sqrt{g}$) that do not satisfy a desired symmetry. If \parlink{helicity\_n}$==0$, modes for which $n \neq 0$ or $m \neq$ \parlink{helicity\_l} are multiplied by \parlink{rippleScale}. If \parlink{helicity\_n}$\neq0$, modes for which $m/n \neq$ \parlink{helicity\_l}/\parlink{helicity\_n} or $n==0$ are multiplied by \parlink{rippleScale}. All other modes remain unchanged.}

\myhrule

\param{EParallelHatSpec\_bcdatFile}
{string}
{``''}
{Only when \parlink{EParallelHatSpec} is non-zero
  and \parlink{RHSmode}==1 \\and \parlink{EParallelHatSpec\_bcdatFile}
  is not == ``''}
...
\begin{gather}
B(\theta_B,\zeta_B) = \sum_{m,n}\mathrm{\texttt{boozer\_bmnc}}(m,n) \cos(m\theta_B-n\mathrm{\texttt{Nperiods}} \zeta_B),
\end{gather}
The first index of \texttt{ Boozer\_bmnc} corresponds to $m \in $[0,\parlink{mmax\_boozer}], and the second corresponds to $n \in $[-\parlink{nmax\_boozer},\parlink{nmax\_boozer}]. Here \parlink{Nperiods} can be specified in the input file.
}
...
{40}
{Only when \parlink{geometryScheme} == 13.}
{Boozer geometry is described for $m \leq $ \texttt{mmax\_boozer}. }
...
{40}
{Only when \parlink{geometryScheme} == 13.}
{Boozer geometry is described for $|n| \leq $ \texttt{nmax\_boozer}. }
...
\param{withNBIspec} {Boolean} {\false} {Whenever \parlink{RHSMode} ==
  1 and \parlink{includePhi1} == \true\\
  ~and \parlink{quasineutralityOption} == 1} {When this is \true, an
...
{real}
{1.0}
{Whenever \parlink{withNBIspec}== \true~ and \parlink{RHSMode} == 1 \\
  and \parlink{includePhi1} == \true~ and \parlink{quasineutralityOption} == 1}
...
If false or \parlink{includePhi1inKineticEquation} == \false, and \parlink{includePhi1} = \true, then $\Phi_1$ is not included in the collision operator. 
...
proportional to $B$ unless explicitly specified using an input file \parlink{EParallelHatSpec\_bcdatFile}.}
...
{Boolean}
{\true}
{Only when \parlink{includePhi1} = \true}
...
{If this parameter is \true, the derivative of the bootstrap current (\parlink{FSABjHatOverRootFSAB2}) is computed with respect to geometry parameters. If \parlink{geometryScheme} == 5, then derivatives will be computed with respect to the quantities \texttt{bmnc}, \texttt{bsupthetamnc}, \texttt{bsupzetamnc}, \texttt{bsubthetamnc}, \texttt{bxubzetamnc}, and \texttt{gmnc} which are found in the wout file. Otherwise, derivatives will be computed with respect to the Boozer spectrum, \texttt{bmnc}.}
...
{\false}
{When \parlink{RHSMode} == 4 or 5}
{If this parameter is \true the derivative of the radial current ($J_r$) is computed with respect to geometry parameters,
\begin{gather}
J_r = \sum_s \texttt{Zs}(s) \texttt{particleFlux\_vm\_rN}(s),
\end{gather}
where \parlink{Zs} and \parlink{particleFlux\_vm\_rN} can be found elsewhere in this document. If \parlink{geometryScheme} == 5, then derivatives will be computed with respect to the quantities \texttt{bmnc}, \texttt{bsupthetamnc}, \texttt{bsupzetamnc}, \texttt{bsubthetamnc}, \texttt{bxubzetamnc}, and \texttt{gmnc} which are found in the wout file. Otherwise, derivatives will be computed with respect to the Boozer spectrum, \texttt{bmnc}.}
...
{\false}
{When \parlink{RHSMode} == 4 or 5}
{If this parameter is \true, the derivative of the total heat flux ($Q$) is computed with respect to geometry parameters,
\begin{gather}
Q = \sum_s \texttt{Zs}(s) \texttt{heatFlux\_vm\_rN}(s),
\end{gather}
where \parlink{Zs} and \parlink{heatFlux\_vm\_rN} can be found elsewhere in this document. If \parlink{geometryScheme} == 5, then derivatives will be computed with respect to the quantities \texttt{bmnc}, \texttt{bsupthetamnc}, \texttt{bsupzetamnc}, \texttt{bsubthetamnc}, \texttt{bxubzetamnc}, and \texttt{gmnc} which are found in the wout file. Otherwise, derivatives will be computed with respect to the Boozer spectrum, \texttt{bmnc}.}
...
{\false}
{When \parlink{RHSMode} == 4 or 5}
{This array is of the same length of {\ttfamily nHats} and other species parameters. If parameter is \true, the derivative of the heat flux (\parlink{heatFlux\_vm\_rN}) will be computed for this species with respect to geometry parameters. If \parlink{geometryScheme} == 5, then derivatives will be computed with respect to the quantities \texttt{bmnc}, \texttt{bsupthetamnc}, \texttt{bsupzetamnc}, \texttt{bsubthetamnc}, \texttt{bxubzetamnc}, and \texttt{gmnc} which are found in the wout file. Otherwise, derivatives will be computed with respect to the Boozer spectrum, \texttt{bmnc}.}
...
{\false}
{When \parlink{RHSMode} == 4 or 5}
{This array is of the same length of {\ttfamily nHats} and other species parameters. If parameter is {\ttfamily .true.}, the derivative of the particle flux (\parlink{particleFlux\_vm\_rN}) will be computed for this species with respect to geometry parameters. If \parlink{geometryScheme} == 5, then derivatives will be computed with respect to the quantities \texttt{bmnc}, \texttt{bsupthetamnc}, \texttt{bsupzetamnc}, \texttt{bsubthetamnc}, \texttt{bxubzetamnc}, and \texttt{gmnc} which are found in the wout file. Otherwise, derivatives will be computed with respect to the Boozer spectrum, \texttt{bmnc}.}
...
{\false}
{When \parlink{RHSMode} == 4 or 5}
{This array is of the same length of {\ttfamily nHats} and other species parameters. If parameter is {\ttfamily .true.}, the derivative of the parallel flow (\parlink{FSABVelocityUsingFSADensityOverRootFSAB2}) will be computed for this species with respect to geometry parameters. If \parlink{geometryScheme} == 5, then derivatives will be computed with respect to the quantities \texttt{bmnc}, \texttt{bsupthetamnc}, \texttt{bsupzetamnc}, \texttt{bsubthetamnc}, \texttt{bxubzetamnc}, and \texttt{gmnc} which are found in the wout file. Otherwise, derivatives will be computed with respect to the Boozer spectrum, \texttt{bmnc}.}
...
{0}
{When \parlink{RHSMode} == 4 or 5}
{Derivatives of outputs specified by \parlink{adjointBootstrapOption}, \parlink{adjointRadialCurrentOption}, \parlink{adjointTotalHeatFluxOption}, \parlink{adjointHeatFluxOption}, \parlink{adjointParticleFluxOption}, and \parlink{adjointParallelFlowOption} will computed for Fourier modes $|n| < $ \texttt{nMaxAdjoint}. }
...
{0}
{When \parlink{RHSMode} == 4 or 5}
{Derivatives of outputs specified by \parlink{adjointBootstrapOption}, \parlink{adjointRadialCurrentOption}, \parlink{adjointTotalHeatFluxOption}, \parlink{adjointHeatFluxOption}, \parlink{adjointParticleFluxOption}, and \parlink{adjointParallelFlowOption} will computed for Fourier modes $m < $ \texttt{mMaxAdjoint}. }
