...
\changed{In section \ref{sec:momentum}, we discuss some observations regarding momentum conservation,
...
\changed{The issues discussed in this section are related to moment equations for mass and energy; momentum has a different status
...
(For example, considering the simple case of a single ion species with $d\Phi_0/d\psi=0$, $F_i$ and $v^2 F_i$ are homogeneous
...
i.e. there is no exact analogue to (\ref{eq:unphysicalMassConservation}), (\ref{eq:unphysicalEnergyConservation1}),
...
for each trajectory model to the full parallel momentum fluid equation. This later equation is
\begin{equation}
???
\end{equation}
First, consider the case of no radial electric field.
The stress tensor is given to the accuracy needed by $\tens{\Pi}_a \approx \tens{\Pi}_{a||}$
where $\tens{\Pi}_{a||} = p_{a \bot} \tens{I} + (p_{a||} - p_{a\bot}) \vect{b}\vect{b}$,
$p_{a||} = m_a \int d^3v\, f_{a1} v_{||}^2$, and $p_{a\bot} = m_a \int d^3v\, f_{a1} v_{\bot}^2/2$.
Then noting the $\int d^3v\, m_a v_{||} (\ldots)$ moment of the streaming and mirror terms is
\begin{eqnarray}
\int d^3v\, m_a v_{||} \left[ v_{||} \vect{b}\cdot\nabla f_{a1} - \frac{1-\xi^2}{2B}v(\vect{b}\cdot\nabla B) \frac{\partial f_{a1}}{\partial \xi} \right]
& = &
\vect{b}\cdot\nabla p_{a||1} + \frac{ p_{a\bot 1} - p_{a || 1}}{B} \vect{b}\cdot\nabla B \nonumber \\
& = &
 -\vect{b} \cdot \left(\nabla \cdot \tens{\Pi}_{a||} \right),
\end{eqnarray}
one can see the drift-kinetic equation (\ref{}) is consistent with the full fluid parallel momentum equation
at least when $E_r=0$.
}
...
\begin{equation}
\tilde{f}_{aE} = -\frac{c}{B} \vect{v}\cdot\vect{b}\times\nabla\Phi \left[ \frac{1}{v} \frac{\partial \bar{f}_a}{\partial v}
-\frac{\xi}{v^2} \frac{\partial \bar{f}_a}{\partial \xi} \right]
\label{eq:fTilde}
\end{equation}
as reflected (using different indepedent variables) in eq (17) of Ref. \cite{Hazeltine} and eq (6) of Ref. \cite{SimakovCattoSecondOrderDKE}.
...
\label{eq:PiE}
\end{equation}
where $n_a V_{a||} = \int d^3v\, \bar{f}_a v_{||}$. Note that this contribution to the stress tensor is a part of the gyroviscosity and is off-diagonal.
Using $\Phi \approx \Phi_0(\psi)$ and $\vect{B} \cdot \left\{ \nabla \cdot \left[ \vect{B}\vect{B}\times\nabla\psi + (\vect{B}\times\nabla\psi) \vect{B} \right] \right\}
=2B^2\nabla\psi\cdot\nabla\times\vect{B}=0$,
we then find the contribution to the parallel momentum equation from (\ref{eq:fTilde})-(\ref{eq:PiE}) is
\begin{equation}
...
\changed{
Finally, let us consider the $\int d^3v\, m_a v_{||} (\ldots)$ moment of the radial electric field
...
\changed{
We close this section by noting another an important difference between the trajectory models
related to the parallel momentum equations.
Consider the case of \emph{quasisymmetry}, which is the condition \cite{PerAndreiPRL} that
$\vect{B}\times\nabla\psi\cdot\nabla B = A(\psi) \vect{B}\cdot\nabla B$ for some flux function
$A(\psi)$.  It was known previously \cite{PerAndreiPRL} that when the $E_r$ terms are not included
...
moment. For the full trajectories, the $d\Phi_0/d\psi$ terms
may be integrated by parts, canceling each other to leave
\begin{equation}
\sum_a Z_a \left< \int d^3v\, f_{a1} \vma\cdot\nabla\psi\right>=0,
\label{eq:noCurrent}
\end{equation}
meaning the radial current vanishes. However, for the DKES and partial trajectory models, the $d\Phi_0/d\psi$ terms
do not cancel, leaving an additional term in (\ref{eq:noCurrent}) proportional to $d\Phi_0/d\psi$,
meaning the radial current is generally nonzero.  Notice that when the full trajectory model is applied in quasisymmetry,
...
of the method here.  
\changed{We do not expect any fundamental new complications to arise
...
In \changed{the typical ``ion root'' scenario}, $E_*$ can be estimated by noting that the
...
\changed{Results for $E_r=0$ are indicated by the $\blacktriangleright$ symbol to the left of each plot.}
...
\changed{
Equivalent to the definition at the end of section \ref{sec:sources} \cite{PerAndreiPRL},
...
for various $y$, collisionality regimes, \changed{radial electric fields, trajectory models}, and collision operators.
